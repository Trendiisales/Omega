...
| `g_nbm_gold_london` | n/a — class has no `shadow_mode` member | true | **CORRECTION 2026-05-08 (S21 build-fix):** previous handoff line claimed NBM "follows kShadowDefault" — that was wrong. `NoiseBandMomentumEngine` (CrossAssetEngines.hpp:2379) has no public `shadow_mode` field; the cross-asset cohort stores shadow_mode on the private `CrossPosition pos_` (CrossAssetEngines.hpp:156) and only `IndexFlowEngine` exposes a `set_shadow_mode()` proxy (IndexFlowEngine.hpp:548). NBM does not. An attempted `g_nbm_gold_london.shadow_mode = true;` belt-and-braces pin (added in S20 source) broke the VPS build with C2039; that line was removed in S21 and replaced with a tombstone comment (engine_init.hpp ≈ line 144). **Practically harmless today:** NBM's entry-dispatch code was removed in the engine-cull audit (96-cell walk-forward + 11-day/3.4M tick sweep, zero profitable configs), so NBM cannot open new positions. The only call site (tick_gold.hpp:2298-2300) runs NBM.on_tick strictly inside `if (has_open_position())` — position-management only. **Safety follow-up:** if a future session re-arms NBM entry dispatch, add a `void set_shadow_mode(bool b) noexcept { pos_.shadow_mode = b; }` proxy to NoiseBandMomentumEngine (parallel to IndexFlowEngine.hpp:548) and call `g_nbm_gold_london.set_shadow_mode(true);` from engine_init.hpp BEFORE reactivating. |
...
3. **NBM shadow gating (CORRECTED 2026-05-08, S21 build-fix):** the original instruction to "pin `g_nbm_gold_london.shadow_mode = true`" was invalid — `NoiseBandMomentumEngine` has no public `shadow_mode` member, only the private `CrossPosition pos_.shadow_mode`. The broken pin was removed from `engine_init.hpp` after it caused a VPS build error (C2039). NBM's entry dispatch is already dead code (engine-cull audit), so live-order risk is currently zero regardless. If NBM is ever re-armed for live signalling, FIRST add a `set_shadow_mode(bool)` proxy to `NoiseBandMomentumEngine` (parallel to `IndexFlowEngine.hpp:548`) and call `g_nbm_gold_london.set_shadow_mode(true);` from `engine_init.hpp` before any `mode=LIVE` flip. See the tombstone comment block in `engine_init.hpp` (≈ line 144) for the full rationale.
