// =============================================================================
// eurusd_london_open_planA_sweep.cpp -- Plan A: baseline + spread/trail grid
// =============================================================================
//
// 2026-05-12 (Claude / Jo): EURUSD-London-Open analogue of the GBPUSD Plan A
//   sweep. Forks the EurusdLondonOpenEngine class (constexpr -> instance
...
//   engine -- against the full EURUSD HistData tick tape. Writes:
//     - eurusd_planA_baseline.csv (per-trade ledger, live constexpr config)
//     - eurusd_planA_stats.csv    (baseline summary)
//     - eurusd_planA_leaderboard.csv (one row per swept combo, sorted by net)
//
// LIVE CONSTEXPR DEFAULTS (from include/EurusdLondonOpenEngine.hpp):
//   MIN_RANGE=0.0008, MAX_RANGE=0.0050, SL_FRAC=0.80, TP_RR=2.0,
//   TRAIL_FRAC=0.30, MFE_TRAIL_FRAC=0.40, MAX_SPREAD=0.00020,
//   MAX_FILL_SPREAD=0.00040, BE_TRIGGER_PTS=0.0006, BE_OFFSET_PTS=0.00015,
//   SAME_LEVEL_BLOCK_PTS=0.0008, session 06-09 UTC.
//
// BUILD
//   g++ -std=c++17 -O3 -DNDEBUG -I include \
//       backtest/eurusd_london_open_planA_sweep.cpp -o /tmp/eurusd_planA
// =============================================================================
...
public:
    // Held-fixed (matches live constexpr in include/EurusdLondonOpenEngine.hpp)
    ...
    // Swept (defaults match live constexpr)
    ...
    // Cost gate ratio. 0.0 disables (we apply costs in close ledger).
    ...
    static bool cost_viable(double spread_pts, double tp_dist_pts,
                 ...
                 int hour_utc) noexcept
    {
        ...
        if (phase == Phase::ARMED) {
            ...
            if (!cost_viable(spread, tp_dist, base_lot, cost_ratio_min)) {
                phase = Phase::IDLE; bracket_high = bracket_low = range = 0.0; return;
            }
...
int main(int argc, char** argv) {
    ...
    std::string baseline_out = "backtest/eurusd_planA_baseline.csv";
    std::string stats_out    = "backtest/eurusd_planA_stats.csv";
    std::string lb_out       = "backtest/eurusd_planA_leaderboard.csv";
    ...
    // Plan A grid (task spec):
    //   MAX_SPREAD ∈ {0.00010, 0.00015, 0.00020 (current), 0.00025, 0.00030, 0.00050}
    //   TRAIL_FRAC ∈ {0.15, 0.20, 0.25, 0.30 (current), 0.40, 0.55}
    //   MFE_TRAIL_FRAC ∈ {0.20, 0.30, 0.40 (current), 0.55, 0.70}
    std::vector<double> spread_grid = {0.00010, 0.00015, 0.00020, 0.00025, 0.00030, 0.00050};
    ...
    // Baseline first (tag=1) -- live EUR constexpr
    combos.push_back({0.00020, 0.30, 0.40, 2.0, 0.80, 1});
    // Sweep
    ...
    for (int i = 0; i < N; ++i) {
        ...
        engines[i].trade_sink = [rp, bfp](const eurfx::TradeRecord& tr) {
            ComboResult& r = *rp;
            r.n_trades += 1;
    ...
    using Engine = eurfx::EurusdLondonOpenEngine;
    constexpr int WIN_CAP = Engine::STRUCTURE_LOOKBACK * 2;
    std::vector<double> ring(WIN_CAP, 0.0);
    int ring_size = 0;
    int64_t tick_idx = 0;
    std::deque<int64_t> dq_max;
    std::deque<int64_t> dq_min;

    eurfx::HistTickStreamer streamer;
    ...
    for (const auto& path : csv_paths) {
        ...
        while (streamer.next(ts_ms, bid, ask)) {
            ...
            const int hour_utc = eurfx::hour_utc_from_ts_ms(ts_ms);
            for (int i = 0; i < N; ++i) {
