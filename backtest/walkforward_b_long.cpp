// walkforward_b_long.cpp
//
// Walk-forward Option B harness — ANCHORED retuning simulation, LONG-ONLY.
//
// Variant of walkforward_b.cpp that ignores Donchian-low (bear) breaks.
// Engine sits flat on bear breaks; only Donchian-high (bull) breaks open
// a position. This reproduces the RANGE_BREAK_HIGH (4H) edge validated
// Jan 2026: PF 2.27 baseline, PF 1.49 under stress, 161 trades over 10
// months. Thesis: gold is structurally long-biased, the short side of a
// symmetric breakout is the loss-generating half.
//
// All non-strategy parts (parser, aggregator, ATR, session gate, picker,
// windows, sweep grid, output schema) are byte-for-byte identical to
// walkforward_b.cpp so this run is directly comparable to the symmetric
// S30 result. The single delta is the removal of the bear-break entry
// branch in run_one().
//
// Three OOS steps:
//   Step 1: IS = W1                  -> OOS = W2
...
// Build (Mac):
//   cd ~/Omega/backtest
//   clang++ -O3 -std=c++17 -o walkforward_b_long walkforward_b_long.cpp
//
// Run XAUUSD (extended Dukascopy windows, matches S30 negative result):
//   ./walkforward_b_long XAUUSD ~/Tick/duka_ticks/XAUUSD_2024-03_2026-03_combined.csv \
//       2024-04-25 2024-10-25 2025-04-25 2025-10-25 2026-04-25
//
// Run XAUUSD (old gold windows for back-comparison):
//   ./walkforward_b_long XAUUSD /Users/jo/tick/2yr_XAUUSD_tick.csv \
//       2023-10-01 2024-04-01 2024-10-01 2025-04-01 2025-09-30
//
// Outputs:
//   walkforward_b_long_<SYMBOL>_picks.csv
//   walkforward_b_long_<SYMBOL>_summary.txt
...
// ============================================================
// ATR + weekend gate (identical to walkforward_b.cpp)
// ============================================================
...
// ============================================================
// Result + run_one (LONG-ONLY variant of walkforward_b.cpp)
//
// SINGLE STRATEGY DELTA from walkforward_b.cpp:
//   The bear-break entry branch ("else if (b.close_bid < dc_low)")
//   has been removed. On a bear break, the engine remains flat.
//   All other logic — bar-level SL/TP resolution, weekend close,
//   final-bar close, ATR sizing, equity tracking, drawdown — is
//   unchanged. Position machinery still supports both signs to
//   keep the diff minimal and the long-side resolution code paths
//   identical to the symmetric harness.
// ============================================================
// ============================================================
...
struct TradeRecord {
    ...
    double sl_mult;
    double tp_mult;
    int direction;                // +1 long, -1 short  (long file: always +1)
    int64_t entry_ms;
    int64_t exit_ms;
    double entry_px;
    double exit_px;
...
static Result run_one(const std::vector<Bar>& bars,
    ...
    for (size_t i = warmup; i < bars.size(); ++i) {
        ...
        if (pos_sign == 0 && in_trading_window(b.bar_open_ms, spec)) {
            ...
            double a = atr[i];
            if (a <= 0.0) continue;
            // LONG-ONLY: only the bull-break branch from walkforward_b.cpp.
            // The bear-break branch ("else if (b.close_bid < dc_low) { ... }")
            // is intentionally absent. dc_low is still computed (cheap, and
            // keeps the loop identical) but unused.
            (void)dc_low;
...
int main(int argc, char** argv) {
    if (argc != 8) {
        ...
        std::cerr << "\nAnchored Option B walk-forward (LONG-ONLY):\n";
        std::cerr << "  Step 1: IS = W1            -> OOS = W2\n";
        std::cerr << "  Step 2: IS = W1+W2         -> OOS = W3\n";
        std::cerr << "  Step 3: IS = W1+W2+W3      -> OOS = W4\n";
        std::cerr << "\nExample (XAUUSD extended Dukascopy):\n";
        std::cerr << "  " << argv[0] << " XAUUSD "
                  << "~/Tick/duka_ticks/XAUUSD_2024-03_2026-03_combined.csv "
                  << "2024-04-25 2024-10-25 2025-04-25 2025-10-25 2026-04-25\n";
        return 1;
    }
    ...
    std::cerr << "Mode:          LONG-ONLY (bear breaks ignored)\n";
    ...
    // Written to walkforward_b_long_<SYM>_trades.csv after the step loop.
    ...
    // ---------- picks.csv ----------
    std::string picks_path = "walkforward_b_long_" + symbol + "_picks.csv";
    ...
    {
        std::string trades_path = "walkforward_b_long_" + symbol + "_trades.csv";
    ...
    // ---------- summary.txt ----------
    std::string sum_path = "walkforward_b_long_" + symbol + "_summary.txt";
    std::ofstream uf(sum_path);
    if (!uf) {
        std::cerr << "ERROR: cannot write " << sum_path << "\n";
        return 3;
    }
    uf << "Walk-forward Option B (anchored, LONG-ONLY) summary - " << symbol << "\n";
    uf << "==================================================\n\n";
    if (symbol == "GER40") {
        uf << "*** CURRENCY: EUR (NOT USD). All P&L figures below are EUR. ***\n\n";
    }
    uf << "Mode: LONG-ONLY (bear Donchian breaks ignored)\n";
