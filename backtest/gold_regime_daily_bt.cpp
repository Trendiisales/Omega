...
#include "../include/GoldRegimeDailyEngine.hpp"
...
struct Driver {
    omega::GoldRegimeDailyEngine engine;
...
static int64_t parse_csv_and_run(Driver& drv, const char* path, const Config& cfg) {
    ...
    while (std::getline(f, line)) {
        ...
        if (n_ticks % 20000000LL == 0) {
            std::fprintf(stderr, "[GRD-SWEEP] %lldM ticks (%s) cum_pnl=$%.2f n=%zu\n",
                (long long)(n_ticks/1000000LL), cfg.label, drv.cum_pnl_usd, drv.trades.size());
        }
    }
    std::fprintf(stderr, "[GRD-SWEEP] Done: ticks=%lld skipped=%lld\n",
                 (long long)n_ticks, (long long)n_skipped);
    return n_ticks;
}

static void apply_config(omega::GoldRegimeDailyEngine& e, const Config& c) {
    // BAR_SECS + ATR_FLOOR are static constexpr in this engine (H4 baked in)
    e.SL_ATR_MULT           = 2.0;
    e.TP_ATR_MULT           = c.tp_atr_mult;
    e.COST_COVER_PTS        = 5.0;       // scaled to H4 magnitudes
    e.BE_BUFFER_PTS         = 1.0;
...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr, "[GRD-SWEEP] CSV: %s\n", csv_path);
    ...
    std::printf("===========================================================================================\n");
    std::printf("  GoldRegimeDaily sweep -- M30 EMA-cross entry + cost-cover BE + trail + trend-flip exit\n");
    ...
    for (size_t i=0; i<configs.size(); ++i) {
        ...
        std::fprintf(stderr, "[GRD-SWEEP] === Cfg %zu/%zu: %s ===\n", i+1, configs.size(), cfg.label);
        ...
        results.push_back(r);
        std::fprintf(stderr,
            "[GRD-SWEEP] Cfg %zu (%s) %.1fs: n=%d wr=%.1f%% pnl=$%.2f pf=%.2f tf=%d tr=%d sl=%d tp=%d\n\n",
    ...
    std::printf("\n=================================================================================================================================\n");
    std::printf("  REGIME H4 RESULTS (sorted by PnL desc) -- runtime %.1f min\n", total_secs/60.0);
    std::printf("=================================================================================================================================\n");
