...
#include "GoldHMM.hpp"
// CFE references g_macroDetector (defined in omega_types.hpp in production).
// Provide a default-constructed instance so its regime() returns the neutral
// default (no macro VIX/DXY feed in backtest) -- matches the live degrade path.
...
static bool parse_tick(const std::string& line, Tick& t) {
    ...
    f.push_back(cur);
    // Schema (l2_xau_vps / l2_data XAUUSD, 17-col):
    //   0 ts_ms, 1 mid, 2 bid, 3 ask, 4 l2_imb, 5 l2_bid_vol, 6 l2_ask_vol,
    //   7 depth_bid_levels, 8 depth_ask_levels, 9 depth_events_total,
    //   10 watchdog_dead, 11 vol_ratio, 12 regime, 13 vpin, 14 has_pos,
    //   15 micro_edge, 16 ewm_drift
    // (FIXED 2026-06-17 audit: original parser used the OLDER 14/16-col
    //  schema with no `mid` column -> read mid-as-bid, bid-as-ask -> 100%
    //  negative spreads. data_integrity_gate confirmed col2=bid col3=ask.)
    if (f.size() < 15) return false;
    try {
        t.ts_ms             = (int64_t)std::stod(f[0]);
        t.bid               = std::stod(f[2]);
        t.ask               = std::stod(f[3]);
        t.l2_imb            = std::stod(f[4]);
        t.l2_bid_vol        = std::stod(f[5]);
        t.l2_ask_vol        = std::stod(f[6]);
        t.depth_bid_levels  = (int)std::stol(f[7]);
        t.depth_ask_levels  = (int)std::stol(f[8]);
        t.watchdog_dead     = (std::stoi(f[10]) != 0);
        t.vol_ratio         = std::stod(f[11]);
        t.ewm_drift         = (f.size() >= 17) ? std::stod(f[16]) : 0.0;
    } catch (...) { return false; }
