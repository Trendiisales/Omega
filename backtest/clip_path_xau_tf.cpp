// clip_path_xau_tf.cpp -- emit per-BAR PATH csv for the XAU TrendFollow zoo
// (XauTrendFollow4h + siblings D1/1h/2h) by driving the REAL engine classes,
// then reconstructing each trade's bar path (closes entry->natural exit) from
// the same bar CSV the engine consumed. Output feeds standalone_clip_overlay.py.
//
// PATH row: trade_id,seq,exit_ms,dir,entry_px,px,atr_pct,bull,cost_rt
// dir from tr.side; entry_px=tr.entryPrice; px=bar close in [entryTs,exitTs];
// atr_pct = Wilder-ATR(14)/price at entry bar (engine's own ATR family);
// bull = close>SMA200 at entry (regime proxy); cost_rt = 2*1.5bps + spread frac.
#include <cstdio>
#include <cstdlib>
#include <cstdint>
#include <cmath>
...
// Wilder ATR(14) as % of close, per bar index (matches engine ATR families)
...
// SMA200 of closes (regime bull proxy)
...
static int idx_at(const std::vector<Bar>&B,int64_t ts){ // nearest bar with ts_sec<=ts
    int lo=0,hi=B.size()-1,r=0;
    while(lo<=hi){int m=(lo+hi)/2; if(B[m].ts_sec<=ts){r=m;lo=m+1;}else hi=m-1;} return r;
}

template<typename Cb> using CV=std::vector<Trade>;

static void emit(FILE* out,int& tid,const std::vector<Trade>&trades,const std::vector<Bar>&B){
    auto ap=atr_pct(B); auto sm=sma200(B);
    const double HALF_SPREAD=0.15; // XAU h4 measured median spread ~0.30 -> half 0.15
    for(const auto& t:trades){
        int ei=idx_at(B,t.entryTs); int xi=idx_at(B,t.exitTs);
        if(xi<ei) xi=ei;
        double atrp = ap[ei];
        int bull = (B[ei].c > sm[ei]) ? 1 : 0;
        // round-trip cost as fraction: 2 * 1.5bps commission + spread/price
        double cost_rt = 2.0*0.00015 + (2.0*HALF_SPREAD)/t.entry_px;
        int seq=0;
...
int main(int argc,char**argv){
    ...
    if(std::string(which)=="d1"){
        ...
        for(size_t i=0;i<h4.size();++i){ const auto&b=h4[i]; int64_t ts=b.ts_sec*1000LL;
            ...
            if(i+1<h4.size()){const auto&nb=h4[i+1];int64_t nt=nb.ts_sec*1000LL;
                eng.on_tick(nb.l-half,nb.l+half,nt,cb); eng.on_tick(nb.h-half,nb.h+half,nt,cb);} }
        barsFor=&h4;
    } else if(std::string(which)=="1h"){
        omega::XauTrendFollow1hEngine eng; eng.shadow_mode=true; eng.enabled=true;
        eng.cell_enable_mask=0x03; eng.lot=0.01; eng.max_spread=1.0; eng.init(); auto cb=mkcb(h1);
        for(size_t i=0;i<h1.size();++i){ const auto&b=h1[i]; int64_t ts=b.ts_sec*1000LL;
            omega::XauTfBar1h bar; bar.bar_start_ms=ts; bar.open=b.o;bar.high=b.h;bar.low=b.l;bar.close=b.c;
            eng.on_h1_bar(bar,b.c-half,b.c+half,0.0,ts,cb);
    ...
    int tid=0; emit(stdout,tid,trades,*barsFor);
    return 0;
}
