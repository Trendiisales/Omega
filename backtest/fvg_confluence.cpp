...
int main(int argc,char**argv){
    ...
    int    CONF=argc>14?atoi(argv[14]):0;        // min # of confluences required (0=baseline FVG)
    string NAME=argc>13?argv[13]:path;
    auto m1=load(path); if((int)m1.size()<500){printf("[%s] few bars\n",NAME.c_str());return 1;}
    auto htf=agg(m1,HTF);
    // ── Confluence inputs on HTF closes (price-only; no volume/L2 in this corpus) ──
    //   C1 TREND   : close on the right side of EMA50
    //   C2 MOMENTUM: 10-bar rate-of-change agrees with the trade direction
    //   C3 SLOPE   : EMA50 slope (vs 5 bars ago) agrees with the direction
    const int EN=50, ROCN=10, SLOPEN=5;
    vector<double> ema(htf.size(),0.0);
    if(!htf.empty()){ double k=2.0/(EN+1.0); ema[0]=htf[0].c;
        for(size_t i=1;i<htf.size();++i) ema[i]=(htf[i].c-ema[i-1])*k+ema[i-1]; }
    auto confluences=[&](int hi,int dir)->int{
        if(hi<EN||hi<ROCN||hi<SLOPEN) return 0;
        int n=0;
        bool c1 = dir>0 ? (htf[hi].c>ema[hi]) : (htf[hi].c<ema[hi]);
        bool c2 = dir>0 ? (htf[hi].c>htf[hi-ROCN].c) : (htf[hi].c<htf[hi-ROCN].c);
        bool c3 = dir>0 ? (ema[hi]>ema[hi-SLOPEN]) : (ema[hi]<ema[hi-SLOPEN]);
        if(c1)n++; if(c2)n++; if(c3)n++; return n;
    };
    // prior-day H/L
    ...
    for(size_t bi=1;bi<m1.size();++bi){
        ...
        for(auto&f:active){ if(f.used)continue;
            ...
            if(CONF>0 && confluences(hidx,f.dir)<CONF) continue;   // confluence gate
    ...
    printf("[%s] HTF=%dm sess=%s cost=%.2f stop=%s k=%.1f maxDol=%.0fA minRR=%.1f exit=%s trail=%.1f CONF>=%d | tr=%d\n",
        NAME.c_str(),HTF,SESS.c_str(),COST,SMODE.c_str(),STOPK,MAXDOL,MINRR,EXIT.c_str(),TRAILA,CONF,(int)trades.size());
