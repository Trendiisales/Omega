// =============================================================================
// pdhl_bt_us30.cpp -- PDH/PDL Reversion Engine 2-year US30 (Dow 30) tick backtest
//
// Runs the EXACT live engine logic from include/PDHLReversionEngine.hpp
// against /Users/jo/tick/dow30_2yr.csv (Tickstory 2-year US30 data).
//
// Tick format: YYYYMMDD,HH:MM:SS,bid_open,ask_open,bid_close,ask_close,volume
// We use bid_open (field 3) and ask_open (field 4) for tick-level price.
//
// No L2 depth available in Tickstory data → runs in DRIFT-FADE PROXY mode
// (same code path live engine uses when l2_real=false).
//
// US30 scale:
//   Price range: ~28000-45000 (vs gold ~1800-3000)
//   Daily range: typically 150-400 pts (vs gold 20-40)
//   Spread:      ~1-3 pts (vs gold 0.3-1.5)
//   Contract:    $5 per pt per lot on most brokers
//
// Build:
//   clang++ -O3 -std=c++17 -o pdhl_bt_us30 pdhl_bt_us30.cpp
// Run:
//   ./pdhl_bt_us30 /Users/jo/tick/dow30_2yr.csv [output.txt]
// =============================================================================
...
static bool parse_line(const char* s, Tick& t) {
    ...
    // Parse 7-field US30 format: date,time,bid_open,ask_open,bid_close,ask_close,volume
    // Use field 3 (bid_open) and field 4 (ask_open) for bid/ask.
    const char* p = s + 18;
    char* end;
    double bid = strtod(p, &end);          // field 3 = bid_open
    if (end == p || *end != ',') return false;
    p = end + 1;
    double ask = strtod(p, &end);          // field 4 = ask_open
    if (end == p) return false;
    // We ignore fields 5, 6, 7 (bid_close, ask_close, volume)
...
struct PDHLParams {
    ...
    // US30-SCALE: gold defaults scaled 10x for index price units
    double DRIFT_FADE_MIN  = 15.0;   // gold=1.5
    double MIN_RANGE_PTS   = 80.0;   // gold=8.0  (US30 daily range typ 150-400)
    double MAX_SPREAD_PTS  = 5.0;    // gold=1.5  (US30 spread typ 1-3)
    double MIN_ATR_PTS     = 5.0;    // gold=1.0  (US30 M1 ATR typ 5-20)
    // PnL calculation: US30 is $5/pt per lot on most brokers (XAUUSD is $100/pt)
    // Our engine uses 100x multiplier (gold convention) so we override in close_trade.
    double RISK_USD        = 30.0;
    double MIN_LOT         = 0.01;
    double MAX_LOT         = 2.00;  // US30 allows bigger size: $5/pt × 2 = $10/pt
...
{
    // US30 PnL: $5 per point per 1.0 lot. Our size is in lots, so pnl = pts * size * 5
    double pnl = (s.is_long ? (exit_px - s.entry) : (s.entry - exit_px)) * s.size * 5.0;
    R.n++;
    if (pnl > 0) R.n_wins++;
    R.pnl_sum += pnl;
                         ...
                         bool want_trades_detail)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        // US30 sizing: $5/pt (gold engine used $100/pt)
        double sz = P.RISK_USD / (sl_pts * 5.0);
