...
// Kaufman efficiency ratio over HTF closes [i-n .. i] (causal). 1=clean trend, 0=chop.
static double erAt(const vector<Bar>&b,int i,int n){ if(i<n||n<1)return -1.0; double dir=fabs(b[i].c-b[i-n].c),vol=0;
    for(int k=i-n+1;k<=i;++k)vol+=fabs(b[k].c-b[k-1].c); return vol>0?dir/vol:0; }
...
int main(int argc,char**argv){
    ...
    // --- ER regime gate (opt-in) : 0=off 1=trend-only(ER>=thr) 2=range-only(ER<thr) ---
    int    REGMODE=argc>18?atoi(argv[18]):0;
    int    ERP=argc>19?atoi(argv[19]):20;        // ER lookback in HTF bars
    double ERT=argc>20?atof(argv[20]):0.30;      // ER threshold
    ...
    size_t fi=0; vector<FVG> active; int hidxRun=-1;
    ...
    for(size_t bi=1;bi<m1.size();++bi){
        ...
        while(hidxRun+1<(int)htf.size()&&htf[hidxRun+1].ts+(int64_t)HTF*60<=now)hidxRun++; // monotonic htf ptr
        int hidx=hidxRun; double atr=hidx>0?atrAt(htf,hidx,14):0;
        for(auto&f:active)if(!f.used&&hidx-f.barIdx>TTL)f.used=true;
        active.erase(remove_if(active.begin(),active.end(),[](const FVG&f){return f.used;}),active.end()); // prune (perf; used are skipped anyway)
        // manage
        if(inT){
        ...
        if(REGMODE>0){ double er=erAt(htf,hidx,ERP);
            if(er<0)continue;                              // not enough HTF history
            if(REGMODE==1 && er<ERT)continue;             // trend-only: skip chop
            if(REGMODE==2 && er>=ERT)continue;            // range-only: skip trend
        }
