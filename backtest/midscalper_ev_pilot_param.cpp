// =============================================================================
// midscalper_ev_pilot_param.cpp -- Param-overridable variant of midscalper_ev_pilot
// =============================================================================
//
// 2026-05-12 (Claude / Jo): Parameter-overridable variant of midscalper_ev_pilot.cpp.
//   Identical engine logic. The four "trail/exit geometry" params (TP_RR,
//   SL_FRAC, MFE_TRAIL_FRAC, MAX_SPREAD) are now CLI-overridable instance
//   members instead of static constexpr. Used by:
//     - Plan A (one-shot): test recommended params 0.75 / 1.0 vs baseline
//     - Plan B (sweep): full TP_RR x MFE_TRAIL_FRAC x SL_FRAC grid
//   All other constants identical to midscalper_ev_pilot.cpp.
//   New CLI: --tp-rr V --sl-frac V --mfe-trail-frac V --max-spread V
//
...
int main(int argc, char** argv) {
    ...
    // Swept engine params (defaults match live config; overridable via CLI):
    double cli_tp_rr           = 4.0;
    double cli_sl_frac         = 0.6;
    double cli_mfe_trail_frac  = 0.55;
    double cli_max_spread      = 2.5;
    double cli_trail_frac      = 0.25;
    for (int i = 1; i < argc; ++i) {
        ...
        else if (std::strcmp(a, "--tp-rr") == 0 && i+1 < argc) cli_tp_rr = std::atof(argv[++i]);
        else if (std::strcmp(a, "--sl-frac") == 0 && i+1 < argc) cli_sl_frac = std::atof(argv[++i]);
        else if (std::strcmp(a, "--mfe-trail-frac") == 0 && i+1 < argc) cli_mfe_trail_frac = std::atof(argv[++i]);
        else if (std::strcmp(a, "--max-spread") == 0 && i+1 < argc) cli_max_spread = std::atof(argv[++i]);
        else if (std::strcmp(a, "--trail-frac") == 0 && i+1 < argc) cli_trail_frac = std::atof(argv[++i]);
        else if (a[0] != '-') all_paths.push_back(a);
    }
    std::printf("[cfg] TP_RR=%.3f SL_FRAC=%.3f MFE_TRAIL_FRAC=%.3f MAX_SPREAD=%.3f TRAIL_FRAC=%.3f\n",
                cli_tp_rr, cli_sl_frac, cli_mfe_trail_frac, cli_max_spread, cli_trail_frac);
    if (all_paths.empty()) {
    ...
    if (pass_mode == 0 || pass_mode == 1) {
        ...
        eng1.TP_RR          = cli_tp_rr;
        eng1.SL_FRAC        = cli_sl_frac;
        eng1.MFE_TRAIL_FRAC = cli_mfe_trail_frac;
        eng1.MAX_SPREAD     = cli_max_spread;
        eng1.TRAIL_FRAC     = cli_trail_frac;
    ...
    if (pass_mode == 0 || pass_mode == 2) {
        ...
        eng2.TP_RR          = cli_tp_rr;
        eng2.SL_FRAC        = cli_sl_frac;
        eng2.MFE_TRAIL_FRAC = cli_mfe_trail_frac;
        eng2.MAX_SPREAD     = cli_max_spread;
        eng2.TRAIL_FRAC     = cli_trail_frac;
