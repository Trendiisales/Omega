// =============================================================================
// fx_scalp_pyramid_bt.cpp -- M5 FX Scalp + Pyramid + Aggressive Trail sweep
//
// S38c FX-adapted clone of gold_scalp_pyramid_bt.cpp. Same engine logic but
// per-pair constants exposed as globals + --pair preset CLI flag.
//
...
enum DataFormat { FMT_UNKNOWN, FMT_LEGACY, FMT_DUKASCOPY, FMT_L2, FMT_FX_MERGED };

// FX merged: ts_ms,bid,ask (bid FIRST, opposite of Dukascopy)
static bool parse_fx_merged(const char* s, Tick& t) {
    ...
    t.ts_ms = ts; t.bid = bid; t.ask = ask;
    return true;
}
...
static double           COST_RT_PTS    = 0.60;   // S38a: realised BB cost @ 0.01 lot (per-pair tunable)
...
static double           ADX_MIN        = 10.0;   // S38c: Wilder ADX gate (0 = off, CLI --adx-min N)
...
// S38c FX: per-pair tunables (set via --pair PRESET or individual flags)
static double           HALF_SPREAD    = 0.00002;  // 0.2 pip for FX majors (was 0.25 for gold)
static double           USD_PER_PT_LOT = 100000.0; // EURUSD: 1 price-unit = 10000 pips × $10/pip-lot
static double           ATR_FLOOR      = 0.00030;  // 3 pip floor for FX majors (was 1.50 for gold)
static double           ATR_CAP        = 0.00500;  // 50 pip cap for FX majors (was 15.0)
static double           SPREAD_CAP     = 0.00050;  // 5 pip max spread (was 0.80 for gold)
static int              g_decimals     = 5;        // 5 for majors, 3 for JPY
static const char*      g_pair         = "EURUSD"; // pair label for output
static constexpr double RISK_DOLLARS   = 50.0;   // max risk per base layer (unchanged)
static constexpr double LOT_CAP        = 0.05;   // max lot per layer (unchanged)
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (fmt == FMT_UNKNOWN) {
            if (!isdigit((unsigned char)line[0])) {
                // Header line -- look for L2 marker / column order
                if (strstr(line, "l2_imb")) {
                    fmt = FMT_L2;
                } else if (strstr(line, "bid,ask") || strstr(line, "ts_ms,bid")) {
                    fmt = FMT_FX_MERGED;  // ts,bid,ask (our merged FX CSVs)
                } else {
                    fmt = FMT_DUKASCOPY;  // ts,ask,bid
                }
                continue;
            }
        ...
        } else if (fmt == FMT_FX_MERGED) {
            parsed = parse_fx_merged(line, tk);
        } else {
...
static Result run_one(const std::vector<M5Bar>& bars, const Config& cfg) {
    ...
    for (size_t i = 0; i < bars.size(); ++i) {
        ...
        // ATR floor: don't trade dead markets (per-pair tunable)
        if (b.atr14 < ATR_FLOOR) continue;
        // ATR cap: don't trade extreme volatility (news spikes)
        if (b.atr14 > ATR_CAP) continue;

        // S38c: ADX directional-strength gate.
        // ADX < threshold = no sustained directional pressure = chop.
        // Asymmetric to ER: a trending move with pullback still scores high
        // (sustained +DM dominance), unlike ER which punishes path-length.
        if (ADX_MIN > 0.0 && b.adx_primed && b.adx14 < ADX_MIN) {
            ++stats_adx_blocked;
            continue;
        }
        ...
        // S38c FX: was hardcoded "range < 0.01" (gold $0.01). For FX use a
        // pair-scaled minimum derived from ATR_FLOOR (1/10 of ATR floor).
        double body = std::fabs(b.close - b.open);
        double range = b.high - b.low;
        if (range < ATR_FLOOR * 0.1) continue;
        ...
        // Spread check: per-pair tunable cap (FX majors ~5 pips, gold 0.80)
        double spread_est = (b.ask_low + b.ask_high) * 0.5 - (b.bid_low + b.bid_high) * 0.5;
        if (spread_est > SPREAD_CAP) continue;
...
int main(int argc, char** argv) {
    ...
    for (int a = 2; a < argc; ++a) {
        ...
        if (arg == "--atr-floor" && a + 1 < argc) ATR_FLOOR    = atof(argv[++a]);
        if (arg == "--atr-cap"   && a + 1 < argc) ATR_CAP      = atof(argv[++a]);
        if (arg == "--spread-cap"&& a + 1 < argc) SPREAD_CAP   = atof(argv[++a]);
        if (arg == "--cost"      && a + 1 < argc) COST_RT_PTS  = atof(argv[++a]);
        // S38c FX: per-pair preset loader
        if (arg == "--pair" && a + 1 < argc) {
            std::string p = argv[++a];
            g_pair = strdup(p.c_str());
            // Defaults already set for xxxUSD majors. Override JPY + cross.
            if (p == "USDJPY") {
                HALF_SPREAD    = 0.002;     // 0.2 pip = 0.002 for JPY (pip = 0.01)
                USD_PER_PT_LOT = 633.0;     // 1 price-unit = 100 pips × $6.33/pip (USDJPY ~158)
                ATR_FLOOR      = 0.03;      // 3 pips JPY
                ATR_CAP        = 0.50;      // 50 pips
                SPREAD_CAP     = 0.05;      // 5 pips
                COST_RT_PTS    = 0.04;      // 4 pips at $0.063/pip @ 0.01 lot = $0.25
                g_decimals     = 3;
            } else if (p == "USDCAD") {
                HALF_SPREAD    = 0.00002;
                USD_PER_PT_LOT = 74000.0;   // 10000 pips × $7.40/pip (CAD ~1.35)
                ATR_FLOOR      = 0.00030;
                ATR_CAP        = 0.00500;
                SPREAD_CAP     = 0.00060;   // CAD wider
                COST_RT_PTS    = 0.00050;   // ~$0.37 RT @ 0.01 lot
                g_decimals     = 5;
            } else if (p == "EURGBP") {
                HALF_SPREAD    = 0.00002;
                USD_PER_PT_LOT = 130000.0;  // GBPUSD ~1.30
                ATR_FLOOR      = 0.00020;   // EURGBP usually quieter
                ATR_CAP        = 0.00300;
                SPREAD_CAP     = 0.00050;
                COST_RT_PTS    = 0.00040;   // ~$0.52 @ 0.01 lot
                g_decimals     = 5;
            } else {
                // EURUSD / GBPUSD / AUDUSD / NZDUSD = xxxUSD majors
                HALF_SPREAD    = 0.00002;
                USD_PER_PT_LOT = 100000.0;
                ATR_FLOOR      = 0.00030;
                ATR_CAP        = 0.00500;
                SPREAD_CAP     = 0.00050;
                COST_RT_PTS    = 0.00040;   // 4 pips @ $0.10/pip = $0.40 RT @ 0.01 lot
                g_decimals     = 5;
            }
        }
    }
    printf("S38c FX config [%s]: COST=%.5f HALF_SPREAD=%.5f USD/PT/LOT=%.0f "
           "ATR=[%.5f..%.5f] SPREAD_CAP=%.5f ADX_MIN=%.1f ER=%s\n",
           g_pair, COST_RT_PTS, HALF_SPREAD, USD_PER_PT_LOT,
           ATR_FLOOR, ATR_CAP, SPREAD_CAP, ADX_MIN,
           g_er_enabled ? "ON" : "OFF");
    fflush(stdout);
