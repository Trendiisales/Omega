// =============================================================================
// h1_bt_minimal.cpp -- Minimal H1 Donchian breakout sweep, XAUUSD 2yr
//
// Purpose: establish whether the edge found on H4 also exists at H1 timeframe,
// or whether the lower timeframe is dominated by intraday noise/mean-reversion
// that kills the breakout signal. H1 = 4x more bars than H4, smaller ATR, so
// spread and slip are a larger % of SL -- edge must be robust enough to survive.
//
// This is a research probe. If H1 shows edge, build MinimalH1Breakout engine.
// If H1 shows no edge, stick with validated H4 engine only.
//
// Strategy (identical to H4 except timeframe):
//   - H1 bar close above Donchian-N high   -> LONG
//   - H1 bar close below Donchian-N low    -> SHORT
//   - SL = sl_mult * H1_ATR behind entry
//   - TP = tp_mult * H1_ATR ahead of entry
...
//
// Sweep (wider grid than H4 to probe multiple H1 regimes):
//   donchian_bars in {10, 20, 30, 50}     // H1=10 is 10hr window; H1=50 is 2 days
//   sl_mult       in {1.0, 1.5, 2.0}
//   tp_mult       in {2.0, 3.0, 4.0}
//   => 36 configs
//
// n-target per config: 200-1000 trades over 2 years (H1 = ~4x the entries of H4).
//
// Build:
//   clang++ -O3 -std=c++17 -o h1_bt_minimal h1_bt_minimal.cpp
// Run:
//   ./h1_bt_minimal /Users/jo/tick/2yr_XAUUSD_tick.csv
// Output:
//   h1_bt_minimal_results.txt         -- full sweep table + best config detail
//   h1_bt_minimal_best_trades.csv     -- trade log for best config
//   h1_bt_minimal_best_equity.csv     -- equity curve for best config
// =============================================================================
...
// =============================================================================
// Phase 1: pre-aggregate all ticks into H1 bars with intra-bar extremes
// =============================================================================
static std::vector<H1BarWithTicks> build_h1_bars(const char* csv_path,
                                                  uint64_t& ticks_ok_out,
                                                  uint64_t& ticks_fail_out,
                                                  int64_t& first_ts_out,
                                                  int64_t& last_ts_out)
{
    std::vector<H1BarWithTicks> bars;
    FILE* f = fopen(csv_path, "r");
    if (!f) { fprintf(stderr, "cannot open %s\n", csv_path); return bars; }

    const int64_t dur_ms = 60LL * 60LL * 1000LL;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (anchor != cur_anchor) {
            // H1 boundary crossed -- finalize and emit
...
// =============================================================================
// Phase 2: run one config against pre-built H1 bars
// =============================================================================
static Result run_one(const std::vector<H1BarWithTicks>& bars, const Config& cfg)
{
    Result r;
    r.cfg = cfg;
    ...
    for (size_t i = 0; i < bars.size(); ++i) {
        const H1BarWithTicks& bt = bars[i];
        const Bar& b = bt.bar;
        ...
        // ---------------------------------------------------------------
        // ENTRY check on H1 bar close, only if no open position
        // ---------------------------------------------------------------
        ...
        // Guard: need i >= cfg.donchian_bars so that k=i-N is >= 0.
                          ...
                          size_t n_bars, double runtime_s)
{
    ...
    fprintf(f, "========================================================================\n");
    fprintf(f, "  h1_bt_minimal -- H1 Donchian breakout sweep, XAUUSD 2yr tick\n");
    fprintf(f, "========================================================================\n\n");
...
int main(int argc, char** argv) {
    ...
    int64_t first_ts = 0, last_ts = 0;
    std::vector<H1BarWithTicks> bars = build_h1_bars(csv_path,
                                                       ticks_ok, ticks_fail,
                                                       first_ts, last_ts);
    ...
    // Find best config by total PnL, requiring n >= 100 for statistical relevance
    // on H1 (higher than H4's n>=30 because H1 produces ~4x more bars, and a config
    // firing <100 trades over 2yrs on H1 likely has severe selection issues).
    // If no config has n>=100, fall back to best PnL overall.
    ...
    write_results       ("h1_bt_minimal_results.txt", all, *best,
                         ticks_ok, ticks_fail, first_ts, last_ts,
                         bars.size(), total_s);
    write_best_trades   ("h1_bt_minimal_best_trades.csv", best->trades);
    write_best_equity   ("h1_bt_minimal_best_equity.csv", best->trades);
    printf("Wrote:\n");
    printf("  h1_bt_minimal_results.txt         (sweep table + best config)\n");
    printf("  h1_bt_minimal_best_trades.csv     (best config trade log)\n");
    printf("  h1_bt_minimal_best_equity.csv     (best config equity curve)\n");
    printf("\nTotal runtime: %.1fs\n", total_s);
