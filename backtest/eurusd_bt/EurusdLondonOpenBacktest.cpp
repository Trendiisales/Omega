// =============================================================================
// EurusdLondonOpenBacktest.cpp
// -----------------------------------------------------------------------------
// Standalone tick-replay backtest for include/EurusdLondonOpenEngine.hpp.
//
// Built per audit-fixes-38a (see docs/SESSION_2026-05-04_FX_BACKTEST_AUDIT.md
// Section 8.1) to backstop the EURUSD reproducibility gap (Item G in the
// carry-forward list). Reads HistData.com ASCII EURUSD tick CSVs, streams
// ticks chronologically through a single EurusdLondonOpenEngine instance,
// and writes per-trade and per-month CSV outputs the post-sweep analysis
// scripts can consume. Single-config-per-binary; the parameter sweep
// wrapper (scripts/eurusd_london_sweep.py) drives multi-config exploration
...
//       -I backtest/eurusd_bt -I include \
//       backtest/eurusd_bt/EurusdLondonOpenBacktest.cpp \
//       -o build/eurusd_london_bt -pthread
//
// RUN:
//   ./build/eurusd_london_bt --ticks ~/Tick/EURUSD \
//       --from 2025-03 --to 2026-04 \
//       --trades bt_eurusd_trades.csv \
//       --report bt_eurusd_report.csv
//
// Tick CSV format (HistData.com ASCII):
//   YYYYMMDD HHMMSSmmm,bid,ask,vol
//
// Cost model (direct USD-quote pair):
//   Spread is implicitly paid: engine uses ask for long entries / bid for
//   long exits (and vice versa) and the harness records pos.entry =
//   bracket_high (ask cross) and exit_px = bid (bid cross). On top of
//   that we add 0.2 pips slip per side and $0.20/lot/side commission to
//   match the BlackBull Prime cost structure.
//
// USD conversion (direct USD-quote pair):
//   pnl_raw = (exit - entry) * size  (in price units * lots)
//   At 0.10 lot, 1 pip = $1, so 1 unit of price = $10,000 at 0.10 lot.
//   Generalising: pnl_usd = pnl_raw * 100000 (regardless of size).
//   No live-mid conversion needed (that's a USDJPY-only step).
// =============================================================================
...
// -- Engine: a backtest-side copy of include/EurusdLondonOpenEngine.hpp.
//    Resolves to backtest/eurusd_bt/EurusdLondonOpenEngine.hpp via the -I
//    backtest/eurusd_bt path. Production engine in include/ stays untouched.
//    The sweep wrapper (scripts/eurusd_london_sweep.py) regenerates this
...
static omega::EurusdLondonOpenEngine g_eng;
...
// UTC London-open session window has the equivalent EST-as-UTC interpretation
// (the 5h offset is treated as a constant; session-window tests are correct
// relative to the timestamps, just shifted by a fixed 5h vs live broker UTC).
// See FX backtest audit Item F MEDIUM for the live-vs-backtest convention gap.
...
// Convert raw price-distance * size pnl into USD for direct USD-quote pairs.
// EURUSD quote convention: USD is the quote currency, so the price IS the
// USD-per-EUR rate and no live-mid conversion is needed.
//   pnl_raw = (exit - entry) * size       (price-units * lots)
//   At 0.10 lot, 10000 base EUR. A 1.0 price move = $10000 USD. So the
//   multiplier from pnl_raw to USD is constant 100000 regardless of size.
//   Verify: 1 pip (0.0001) at 0.10 lot = 0.0001 * 0.10 * 100000 = $1. Correct.
static double convert_to_usd(double pnl_raw) noexcept {
    return pnl_raw * 100000.0;
}

// Cost model for direct USD-quote FX pairs (EUR/GBP/AUD/NZD):
...
//                exits and vice versa; harness records bracket_high as fill
//                for long, bracket_low for short, both ask-cross/bid-cross).
//   For 0.10 lot: total round-trip = 0.4 pip * $1/pip + $0.04 = $0.44.
//   For 0.20 lot: total round-trip = 0.4 pip * $2/pip + $0.08 = $0.88.
static double apply_costs_usd(double pnl_usd, double size, bool /*is_long*/) noexcept {
    const double slip_pips_per_side = 0.2;
    // Per side slip in USD: (size/0.10) * 1 USD/pip * 0.2 pips
    const double slip_usd = slip_pips_per_side * (size / 0.10) * 1.0 * 2.0; // both sides
...
collect_monthly_files(const std::string& root, const std::string& from, const std::string& to) {
    // root contains HISTDATA_COM_ASCII_EURUSD_T<YYYYMM>/ subdirs.
    ...
    for (auto& e : fs::directory_iterator(root)) {
        ...
        // Expect name like HISTDATA_COM_ASCII_EURUSD_T202503 (or with trailing " (1)")
        const std::string prefix = "HISTDATA_COM_ASCII_EURUSD_T";
        ...
        for (auto& f : fs::directory_iterator(e.path())) {
            ...
            if (fname.find("DAT_ASCII_EURUSD_T_") == 0 &&
                fname.size() >= 4 &&
...
int main(int argc, char** argv) {
    ...
    std::string trades_csv = "bt_eurusd_trades.csv";
    std::string report_csv = "bt_eurusd_report.csv";
