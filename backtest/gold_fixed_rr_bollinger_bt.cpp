// =============================================================================
// gold_fixed_rr_bollinger_bt.cpp -- alternative mechanism: hard SL + hard TP, no trail
// =============================================================================
// Build: clang++ -O3 -std=c++17 -Iinclude -o backtest/gold_fixed_rr_bollinger_bt backtest/gold_fixed_rr_bollinger_bt.cpp
// =============================================================================
...
static void apply_config(omega::GoldFixedRREngine& e, const Config& c) {
    ...
    e.LONG_ONLY           = false;      // Bollinger fade -- need both directions
    e.TIGHT_SESSION       = false;      // chop happens any session; don't filter
    e.ATR_FLOOR_OVERRIDE  = 1.5;        // moderate ATR floor
    e.VOL_EXPANSION       = false;      // mean-revert wants NORMAL vol, not spike
    e.BOLLINGER_FADE      = true;       // mean-reversion entry mode
    e.BOLLINGER_PERIOD    = 20;
    e.BOLLINGER_STD_MULT  = 2.0;
    e.shadow_mode         = true;
    e.enabled             = true;
}
