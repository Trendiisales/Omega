...
static bool load_csv_into(const char* path, std::vector<Tick>& out) {
    fprintf(stderr,"[LOAD] %s ...\n",path); fflush(stderr);
    size_t before = out.size();
    std::ifstream f(path);
    if (!f) { fprintf(stderr, "[LOAD] FAIL: Cannot open %s\n", path); fflush(stderr); return false; }
    ...
    if (col_bid<0||col_ask<0) { fprintf(stderr,"[LOAD] FAIL: No bid/ask cols in %s\n", path); fflush(stderr); return false; }
    ...
    fprintf(stderr,"[LOAD] %s: %zu ticks\n",path,out.size()-before); fflush(stderr);
    return true;
}
...
int main(int argc, char* argv[]) {
    if (argc<2) { puts("Usage: cfe_sweep_v2 <csv> [csv ...]"); return 1; }

    auto t0 = std::chrono::steady_clock::now();

    // ── PHASE 1: LOAD ALL FILES ────────────────────────────────────────────
    fprintf(stderr,"=== PHASE 1: LOADING %d FILES ===\n",argc-1); fflush(stderr);
    std::vector<Tick> ticks;
    ticks.reserve(4000000);
    for (int i=1; i<argc; ++i) load_csv_into(argv[i], ticks);
    fprintf(stderr,"\n=== LOAD DONE: %zu total ticks ===\n\n", ticks.size()); fflush(stderr);
    if (ticks.empty()) { fprintf(stderr,"No ticks loaded\n"); return 1; }

    // Verify ATR non-zero at tick 300 of the combined stream
    if (ticks.size()>300 && ticks[300].atr<=0) {
        fprintf(stderr,"WARN: ATR is 0 at tick 300 — fallback may be slow to warm\n");
    }

    // ── PHASE 2: PER-DAY BASELINE with CLAIMED default config ──────────────
    // Default config = first point of the grid (rn=20 rt=2 dm=0.3 st=0.3 sm=10s sl=0.4 tp=2 cd=15s mh=300s tr=F)
    fprintf(stderr,"=== PHASE 2: PER-DAY BASELINE ===\n"); fflush(stderr);
    printf("\n========== PER-DAY BASELINE (first-grid-point default config) ==========\n");
    printf("%-50s %6s %5s %5s %9s\n","File","Ticks","T","W","PnL");
    printf("%s\n",std::string(80,'-').c_str());
    double agg_pnl=0; int agg_T=0, agg_W=0;
    P default_p{20, 2.0, 15.0, 0.3, 0.3, 10000LL, 0.4, 2.0, 15000LL, 300000LL, false};
    for (int i=1; i<argc; ++i) {
        std::vector<Tick> day; day.reserve(500000);
        if (!load_csv_into(argv[i], day) || day.empty()) continue;
        Eng eng(default_p);
        for (auto& tk : day) eng.tick(tk.ms, tk.bid, tk.ask, tk.drift, tk.atr, default_p);
        eng.fin(day.back().ms, default_p);
        const char* fn = argv[i];
        const char* slash = strrchr(fn,'\\'); if (!slash) slash = strrchr(fn,'/');
        if (slash) fn = slash+1;
        printf("%-50s %6zu %5d %5d %+9.2f\n", fn, day.size(), eng.n_trades, eng.n_wins, eng.total_pnl);
        fflush(stdout);
        agg_T += eng.n_trades; agg_W += eng.n_wins; agg_pnl += eng.total_pnl;
        fprintf(stderr,"  [DAY %d/%d] %s: T=%d PnL=%.2f\n",i,argc-1,fn,eng.n_trades,eng.total_pnl); fflush(stderr);
    }
    printf("%s\n",std::string(80,'-').c_str());
    printf("%-50s %6s %5d %5d %+9.2f  WR=%.1f%%\n", "AGGREGATE","-",agg_T,agg_W,agg_pnl,
           agg_T>0?100.0*agg_W/agg_T:0.0);
    printf("\n"); fflush(stdout);

    // ── PHASE 3: REDUCED GRID SWEEP (288 configs) ──────────────────────────
    // Down from 5184 to 288 for VPS speed. Keeps the high-signal axes.
    const int    rn_v[]  = {20,30};                     // 2
    const double rt_v[]  = {2.0,4.0};                   // 2
    const double rm_v[]  = {15.0};                      // 1
    const double dm_v[]  = {0.3,0.5,0.8};               // 3
    const double st_v[]  = {0.3,0.5};                   // 2
    const int64_t sm_v[] = {10000,30000};               // 2 (was 3)
    const double sl_v[]  = {0.4,0.8};                   // 2 (was 3)
    const double tp_v[]  = {2.0,3.0,4.0};               // 3
    const int64_t cd_v[] = {30000};                     // 1 (was 2)
    const int64_t mh_v[] = {300000};                    // 1 (was 2)
    const bool   tr_v[]  = {false,true};                // 2
    // total = 2*2*1*3*2*2*2*3*1*1*2 = 288

    constexpr long TOTAL = 2*2*1*3*2*2*2*3*1*1*2;
    fprintf(stderr,"\n=== PHASE 3: SWEEP (%ld configs on %zu ticks) ===\n",TOTAL,ticks.size()); fflush(stderr);
    printf("Testing %ld combinations on %zu ticks...\n", TOTAL, ticks.size());
    ...
    for (bool   tr  : tr_v)
    {
        ...
        if (++done % 24 == 0) {
            ...
            fprintf(stderr,"  [SWEEP] %ld/%ld (%.0f%%)  best=$%+.2f\n", done, TOTAL, 100.0*done/TOTAL, best);
            fflush(stderr);
        }
    }
    fprintf(stderr,"=== SWEEP DONE (%ld configs, %zu profitable) ===\n", done, results.size()); fflush(stderr);
