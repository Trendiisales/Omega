...
int main(int argc,char**argv){
    ...
    int    MAXAGE=argc>12?atoi(argv[12]):24;
    int    TRENDEMA=argc>13?atoi(argv[13]):0;   // SMA(TRENDN) HTF trend gate (0=off) -- matches engine TRENDN
    ...
    // ALIGNED with the HTF trend (block counter-trend, e.g. shorting into a rally).
    // SMA(TRENDN) over HTF closes -- matches the engine's TRENDN gate exactly.
    vector<double> htfEma(htf.size(),0.0);
    if(TRENDEMA>0){ double run=0.0; for(size_t i=0;i<htf.size();++i){ run+=htf[i].c;
        if((int)i>=TRENDEMA) run-=htf[i-TRENDEMA].c;
        int cnt=(int)i+1<TRENDEMA?(int)i+1:TRENDEMA; htfEma[i]=run/cnt; } }
    // MACD(12,26,9) on HTF closes -- matches the engine's MACD_GATE. macdBull[i]=macd>signal.
    vector<char> macdBull(htf.size(),1);
    { double e12=0,e26=0,sig=0; bool init=false; double k12=2.0/13,k26=2.0/27,k9=2.0/10;
      for(size_t i=0;i<htf.size();++i){ double c=htf[i].c;
        if(!init){ e12=e26=c; init=true; } else { e12=(c-e12)*k12+e12; e26=(c-e26)*k26+e26; }
        double macd=e12-e26; sig=(i==0)?macd:(macd-sig)*k9+sig; macdBull[i]=(macd>=sig)?1:0; } }
    // prior-day H/L
    ...
    for(size_t bi=1;bi<m1.size();++bi){
        ...
        if(inT){
            ...
            // reversal-exit: cut early at close when the HTF MACD flips AGAINST us,
            // but ONLY before the trade reaches +1R (rescue failing trades; never cut
            // a winner). This is the intraday-reversal containment for counter-trend bleed.
            bool hitRev = REVEXIT && hidx>=0 && peakFav<cur.r && (cur.dir>0? !macdBull[hidx] : (bool)macdBull[hidx]);
            if(hitSL||hitTP||hitTrail||hitRev){
                double exit = hitSL?cur.sl : (hitTP?cur.tp : (hitTrail?trailStop : Cl));
        ...
        for(auto&f:active){ if(f.used)continue;
            ...
            if(TRENDEMA>0 && hidx>=0){ double te=htfEma[hidx];          // trend gate: no counter-trend
                if(f.dir>0 && px<=te) continue; if(f.dir<0 && px>=te) continue; }
