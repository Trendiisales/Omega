...
int main(int argc, char** argv) {
    ...
    std::vector<Tick> ticks; ticks.reserve(4000000);
    ...
    printf("\n========== PER-DAY BASELINE (default config) ==========\n");
    printf("%-50s %6s %5s %4s %7s %6s %7s %3s %3s %3s\n",
           "File","Ticks","T","W","PnL","WR%","MaxDD","TP","SL","TMO");
    printf("%s\n",std::string(110,'-').c_str());
    double agg_pnl=0; int agg_T=0, agg_W=0;
    for (int i=1;i<argc;++i) {
        std::vector<Tick> day; day.reserve(500000);
        if (!load_csv(argv[i],day) || day.empty()) continue;
        Config base;
        Stats s = run(day, base);
        const char* fn = argv[i];
        const char* slash = strrchr(fn,'\\'); if (!slash) slash = strrchr(fn,'/');
        if (slash) fn = slash+1;
        printf("%-50s %6zu %5d %4d %7.2f %5.1f %7.2f %3d %3d %3d\n",
               fn, day.size(), s.T, s.W, s.pnl, s.wr(), s.max_dd, s.tp, s.sl, s.T-s.tp-s.sl);
        fflush(stdout);
        agg_pnl += s.pnl; agg_T += s.T; agg_W += s.W;
        fprintf(stderr,"  [DAY %d/%d] %s: T=%d PnL=%.2f\n",i,argc-1,fn,s.T,s.pnl); fflush(stderr);
    }
    printf("%s\n",std::string(110,'-').c_str());
    printf("%-50s %6s %5d %4d %7.2f %5.1f\n",
           "AGGREGATE", "-", agg_T, agg_W, agg_pnl,
           agg_T>0 ? 100.0*agg_W/agg_T : 0.0);
    printf("\n"); fflush(stdout);

    fprintf(stderr,"\n=== PHASE 3: COMBINED BASELINE ===\n"); fflush(stderr);
    {
        Config base;
        Stats bs=run(ticks,base);
        printf("BASELINE (combined): T=%d WR=%.1f%% PnL=$%.2f Avg=$%.2f MaxDD=$%.2f TP=%d SL=%d\n\n",
    ...
    fprintf(stderr,"\n=== PHASE 4: SWEEP (288 configs) ===\n"); fflush(stderr);
    int cfg_count=0;
    const int total_cfgs = 288;
    for (auto sess : {ALL_SESSIONS, LONDON_NY, LONDON_ONLY})
    for (bool  kw  : {false, true})
    for (bool  ll  : {false, true})
    for (double ov : {99.0, 0.30})
    for (double sr : {70.0, 78.0})
    for (double lr : {30.0, 25.0})
    for (int   hk  : {0, 1, 2})
    {
        Config c;
        c.session_mode        = sess;
        ...
        ++cfg_count;
        if (cfg_count % 24 == 0) {
            fprintf(stderr,"  [SWEEP] %d/%d (%.0f%%)\n",
                    cfg_count, total_cfgs, 100.0*cfg_count/total_cfgs);
            fflush(stderr);
        }
    }
    fprintf(stderr,"=== SWEEP DONE (%d configs) ===\n", cfg_count); fflush(stderr);
