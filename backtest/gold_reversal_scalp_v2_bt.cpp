// =============================================================================
// gold_reversal_scalp_v2_bt.cpp -- GoldReversalScalpV2Engine sweep harness
// =============================================================================
//
...
//
// Build:  clang++ -O3 -std=c++17 -Iinclude -o backtest/gold_reversal_scalp_v2_bt \
...
#include "../include/GoldReversalScalpV2Engine.hpp"
...
struct Driver {
    omega::GoldReversalScalpV2Engine engine;
...
static int64_t parse_csv_and_run(Driver& drv, const char* path, const Config& cfg) {
    std::ifstream f(path);
    if (!f.is_open()) {
        std::fprintf(stderr, "[GRS2-SWEEP] FAIL: cannot open %s\n", path);
        return -1;
    }
    ...
    while (std::getline(f, line)) {
        ...
        if (n_ticks % 20000000LL == 0) {
            std::fprintf(stderr,
                "[GRS2-SWEEP] %lldM ticks (%s) cum_pnl=$%.2f n_trades=%zu\n",
                (long long)(n_ticks / 1000000LL),
                cfg.label, drv.cum_pnl_usd, drv.trades.size());
        }
    }
    std::fprintf(stderr, "[GRS2-SWEEP] Done: ticks=%lld skipped=%lld\n",
                 (long long)n_ticks, (long long)n_skipped);
    return n_ticks;
}

static void apply_config(omega::GoldReversalScalpV2Engine& e, const Config& c) {
    e.LOOKBACK = 8;
    e.SL_ATR_MULT = 2.0;
    e.TP_ATR_MULT = 3.0;
...
int main(int argc, char** argv) {
    ...
    std::fprintf(stderr, "[GRS2-SWEEP] CSV: %s\n", csv_path);
    ...
    std::printf("===================================================================================\n");
    std::printf("  GoldReversalScalpV2 sweep -- M1-bar reversal detector\n");
    ...
    for (size_t i = 0; i < configs.size(); ++i) {
        ...
        std::fprintf(stderr, "[GRS2-SWEEP] === Cfg %zu/%zu: %s ===\n", i+1, configs.size(), cfg.label);
        ...
        results.push_back(r);
        std::fprintf(stderr,
            "[GRS2-SWEEP] Cfg %zu (%s) done %.1fs: n=%d wr=%.1f%% pnl=$%.2f pf=%.2f rev=%d sl=%d tp=%d\n\n",
    ...
    std::printf("\n=================================================================================================================================\n");
    std::printf("  V2 RESULTS (sorted by PnL desc) -- runtime %.1f min\n", total_secs/60.0);
    std::printf("=================================================================================================================================\n");
