// =============================================================================
// eurusd_london_open_planB_sweep.cpp -- Plan B: TP/SL/Trail/MFE 4D sweep
// =============================================================================
//
// 2026-05-12 (Claude / Jo): EURUSD-London-Open analogue of GBPUSD Plan B.
//   4D grid: TP_RR x SL_FRAC x TRAIL_FRAC x MFE_TRAIL_FRAC.
//   MAX_SPREAD pinned to Plan A best/live (0.00020).
//
//   Also computes per-engine OOS stats on the last N input files (default 4).
//
// USAGE
//   ./eurusd_london_open_planB_sweep <list of monthly HistData csvs ...>
//       [--oos-trailing 4]
//       [--max-spread 0.00020]
...
//       backtest/eurusd_london_open_planB_sweep.cpp -o /tmp/eurusd_planB
// =============================================================================
...
public:
                 ...
                 int hour_utc) noexcept
    {
        ...
        if (phase == Phase::PENDING) {
            ...
            if (would_fill_long)  { _confirm_fill(true,  bracket_high, spread, now_ms, hour_utc); return; }
            if (would_fill_short) { _confirm_fill(false, bracket_low,  spread, now_ms, hour_utc); return; }
...
private:
    ...
    void _confirm_fill(bool is_long, double fill_px,
...
struct HistTickStreamer {
    ...
    bool open(const char* p) {
        f.open(p); if (!f.is_open()) return false;
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::fprintf(stderr, "usage: %s <csv...> [--oos-trailing N] [--max-spread V] "
            "[--leaderboard-out P] [--oos-out P]\n", argv[0]);
        return 2;
    }
    std::vector<std::string> csv_paths;
    int oos_trailing = 4;
    double max_spread = 0.00020;
    std::string lb_out  = "backtest/eurusd_planB_leaderboard.csv";
    std::string oos_out = "backtest/eurusd_planB_oos.csv";
    ...
    // 4D grid (task spec): TP x SL x TF x MT
    std::vector<double> tp_grid = {1.5, 2.0, 2.5, 3.0};
    std::vector<double> sl_grid = {0.60, 0.70, 0.80, 0.90, 1.00};
    std::vector<double> tf_grid = {0.20, 0.30, 0.45, 0.55};
    std::vector<double> mt_grid = {0.30, 0.40, 0.55, 0.70};
    ...
    for (int i = 0; i < N; ++i) {
        ...
        Result* rp = &results[i];
        engines[i].trade_sink = [rp, oos_cutoff_ms](const eurfx::TradeRecord& tr) {
            Result& r = *rp;
