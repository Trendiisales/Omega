...
| VWAPReversion SP/NQ/GER40/EURUSD (`g_vwap_rev_*`) | own flag | ✅ | ✅ | n/a | n/a | n/a | LIVE-SHADOW | engine_init.hpp:447/450/453/467 — RE-ENABLED S17 (commit 1573b7a). Trading mode is SHADOW so paper-only until LIVE flip. EURUSD instance fully tuned in S18: `EXTENSION_THRESH_PCT=0.12` + `COOLDOWN_SEC=120` + `MAX_EXTENSION_PCT=0.40` + `MAX_HOLD_SEC=600` (chosen to maintain ~3.3x threshold-to-max ratio of indices; ~44 pips daily-range upper bound). Re-tune from fresh shadow tape after 2-4 weeks. |
