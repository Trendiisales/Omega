...
# Only delete the specific files that cause stale binary issues:
# CMakeCache.txt forces reconfigure, .obj/.pch force full recompile.
if (Test-Path "$OmegaDir\build\CMakeCache.txt") {
    Remove-Item "$OmegaDir\build\CMakeCache.txt" -Force -ErrorAction SilentlyContinue
}
# Delete obj/pch files -- these are regenerated by the build
