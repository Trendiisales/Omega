...
//
// LOG NAMESPACE:
//   [GRS2]. tr.engine = "GoldReversalScalpV2". tr.regime = "M5_REVERSAL_V2".
// =============================================================================
...
class GoldReversalScalpV2Engine {
public:
    static constexpr int BAR_SECS    = 300;  // 5min for entry
                 ...
                 const CloseCallback* ext_close = nullptr)
    {
        ...
        std::snprintf(buf, sizeof(buf),
            "[GRS2] OPEN %s entry=%.2f sl=%.2f tp=%.2f size=%.3f atr=%.2f spread=%.2f shadow=%s\n",
...
private:
                         ...
                         const CloseCallback* ext_close)
    {
        ...
        char buf[384];
        std::snprintf(buf, sizeof(buf),
            "[GRS2] CLOSE %s entry=%.2f exit=%.2f pnl=$%.2f size=%.3f "
        ...
        tr.engine     = "GoldReversalScalpV2";
        tr.symbol     = "XAUUSD";
        tr.side       = m_pos.is_long ? "LONG" : "SHORT";
        tr.regime     = "M5_REVERSAL_V2";
