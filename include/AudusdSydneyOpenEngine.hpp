...
public:
    ...
    //   safe fallback.
    AudusdSydneyOpenEngine() noexcept {
        _try_load_range_history();
        // S62 2026-05-13: rehydrate post-trade same-level block state so a
        //   service restart inside the post-SL window does NOT silently
        //   bypass the re-arm block.
                 ...
                 bool   l2_real     = false) noexcept
    {
            ...
            if ((would_fill_long || would_fill_short) && spread > MAX_FILL_SPREAD) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[AUD-SYD-OPEN] FILL_SPREAD_REJECT spread=%.5f max=%.5f side=%s -- aborting bracket\n",
            ...
            if (m_sl_price > 0.0 && now_s < m_sl_cooldown_ts) {
                if (std::fabs(w_hi - m_sl_price) < SAME_LEVEL_BLOCK_PTS ||
                    ...
                    // S62 2026-05-13: emit log line so this block is greppable.
                    {
                        char _buf[320];
                        snprintf(_buf, sizeof(_buf),
                            "[AUD-SYD-OPEN] SAME_LEVEL_BLOCK side=SL sl_price=%.5f "
            ...
            if (m_win_exit_price > 0.0 && now_s < m_win_exit_block_ts) {
                if (std::fabs(w_hi - m_win_exit_price) < SAME_LEVEL_BLOCK_PTS ||
                    std::fabs(w_lo - m_win_exit_price) < SAME_LEVEL_BLOCK_PTS) {
                    {
                        char _buf[320];
                        snprintf(_buf, sizeof(_buf),
                            "[AUD-SYD-OPEN] SAME_LEVEL_BLOCK side=WIN win_price=%.5f "
                            "w_hi=%.5f w_lo=%.5f radius=%.5f remaining_s=%lld\n",
        ...
        if (phase == Phase::ARMED) {
            ...
            if ((int)m_range_history.size() < EXPANSION_MIN_HISTORY) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[AUD-SYD-OPEN] COLD_START_BLOCK range=%.5f hist=%d/%d -- skipping fire\n",
                ...
                if (range < pct_threshold || range < abs_threshold) {
                    {
                        char _buf[384];
                        snprintf(_buf, sizeof(_buf),
                            "[AUD-SYD-OPEN] ATR_GATE_FAIL range=%.5f median=%.5f "
            ...
            if (!ExecutionCostGuard::is_viable(
                    "AUDUSD", spread, tp_dist, base_lot, 1.5))
            {
                phase = Phase::IDLE;
                bracket_high = bracket_low = range = 0.0;
                return;
...
private:
...
#ifdef _WIN32
        return "C:\\Omega\\state\\audusd_sydney_open_range_history.csv";
#else
        return "state/audusd_sydney_open_range_history.csv";
#endif
    }
...
#ifdef _WIN32
        return "C:\\Omega\\state\\audusd_sydney_open_post_trade_block.csv";
#else
        return "state/audusd_sydney_open_post_trade_block.csv";
#endif
    }
    ...
    void _try_load_post_trade_block() noexcept {
        ...
        if (!f) {
            std::printf("[AUD-SYD-OPEN] POST_TRADE_BLOCK_LOAD no_file path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        char line[256];
    ...
    void _save_post_trade_block() noexcept {
        ...
        if (!f) {
            std::printf("[AUD-SYD-OPEN] POST_TRADE_BLOCK_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
    ...
    void _try_load_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "r");
        if (!f) {
            std::printf("[AUD-SYD-OPEN] RANGE_HIST_LOAD no_file path=%s (cold-start)\n", path);
            std::fflush(stdout);
            return;
        }
        char line[128];
        ...
        if (saved_ts <= 0 || (now_s - saved_ts) > RANGE_HIST_STALENESS_S) {
            std::printf("[AUD-SYD-OPEN] RANGE_HIST_LOAD stale saved=%lld age_s=%lld (cold-start)\n",
        ...
        std::printf("[AUD-SYD-OPEN] RANGE_HIST_LOAD ok n=%d age_s=%lld\n",
    ...
    void _save_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "w");
        if (!f) {
            std::printf("[AUD-SYD-OPEN] RANGE_HIST_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
