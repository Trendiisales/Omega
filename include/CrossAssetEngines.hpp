...
// Backtest mode: read simulated time from OmegaTimeShim's g_sim_now_ms.
// The harness calls omega::bt::set_sim_time(r.ts_ms) before each on_tick(),
// so all session/cooldown/entry-stamp logic in the cross engines runs on
// historical tick time, not wall-clock. Production code never includes the
// shim and falls through to the live-time #else branch.
