...
static void init_engines(const std::string& cfg_path)
{
    ...
    // datasets. CAVEAT: validated only in the 2025-26 bull regime → SHADOW only.
    {
        ...
        g_idx_bear_short_nas.enabled     = true;   // VINDICATED S-2026-06-22 by the REAL ENGINE on REAL NSXUSD 2022 ticks (96M ticks via on_tick, backtest/ibs_real_engine_nsx2022.cpp): n=22 WR45% PF1.70 net +1970pt, BOTH HALVES POSITIVE (H1 PF1.73 / H2 PF1.65) -- confirms the S-06-15 PF~1.6 2022 claim. NOTE: an earlier same-session disable was REVERTED -- it rested on a D1-SCALED PORT (DON20/EMA50, wrong timeframe, showed -4410pt) and a buggy python H1 re-implementation (n=2); both diverged from the real engine. Lesson: drive the real engine class, do not re-implement. Selective Donchian-48 breakdown + sustained-bear gate + fixed-2R is NOT the naive short-every-bear-day the BlendBook proxy killed -- the selectivity is the edge. Bear HEDGE, shadow. FULL faithful picture (real engine): 2022 bear +1970pt PF1.70 both-halves; 2024-26 bull -977pt PF0.84 n=29 (gate arms on corrections that recover = the BlendBook bull-bleed, real but smaller than the bear edge); net +993pt over the cycle. IMPROVEMENT TARGET: cut the bull bleed via the IndexRiskGate / tighter sustained-bear gate before any live size. backtest/ibs_real_engine_nsx2022.cpp + /tmp/ibs_bull_check.cpp.
