...
static void init_engines(const std::string& cfg_path)
{
    ...
    // shadow_mode=true always. To go live: validate shadow signals then set false.
    {
        ...
        //    STANDALONE (feedback-companion-independent-engine).
        {
            ...
            // BE-entry mimic on the two live SurvivorPortfolio cells. Trigger = one-way
            // on_trend_open from Cell::evaluate_signal; feed = the cell's NATIVE H4 close
            // (SurvivorPortfolio.hpp on_tick, fed BEFORE evaluate_signal so a fresh leg first
            // sees the NEXT bar = mimic_ladder_overlay seq0-skip semantics). Validated
            // backtest/clip_path_survivor.cpp (REAL Portfolio, live S-2026-07-08c config,
            // parity n=445 == audited survivor_gated_bt) + mimic_ladder_overlay:
            //   XAU_4h_DonchN20  PROFIT-CONFIRMED be1.0/arm0.25/lc2/cap30/gb10: +14.6%/leg
            //     PF2.05 DD-3.2% n=60, all-6 PASS, 2x-cost PF1.81 PASS, plateau 21/27.
    ...
    // datasets. CAVEAT: validated only in the 2025-26 bull regime → SHADOW only.
    {
        ...
        g_connors_nas.enabled     = true;   // S-2026-07-24b RE-ENABLED (operator: cornerstone engine; the
                                            // S-07-24 disable was a bad firefight call). Routes NAS100->QQQ
                                            // (STK/SMART) — the DELIBERATE S-07-22j ETF-proxy under the
                                            // account's stocks permission; NOT MNQ (which margin-walls the AU
                                            // account against the operator's "trade smaller on indices" order).
                                            // The reconnect order-storm is now BACKSTOPPED by the circuit
                                            // breaker (9303b06e, live: rate 25/s + per-sym-unfilled 8, sticky)
                                            // + reject-reconcile (on_reject clears intent so a rejected order
                                            // stops retrying). QQQ/STK fill is the system-wide 0-fills matter,
                                            // tracked separately — the engine itself is no longer gated.
