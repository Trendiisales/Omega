...
static void init_engines(const std::string& cfg_path)
{
    ...
    // shadow_mode=true always. To go live: validate shadow signals then set false.
    {
        ...
        //   SHADOW book, judged STANDALONE, deploy-forward, per-clip RT cost debited.
        {
            ...
            il.finalize_all();
            printf("[OMEGA-INIT][SEED] INDEX upjump LADDER wired: US500(W24/2.0) NAS100(W24/1.5) GER40(W12/1.5 BULL-GATED) M2K(W24/1.5 BULL-GATED, feed via bridge --symbols M2K), ALL IBKR-futures seed+live (S-2026-07-09 complete migration), %zu H1 warmup bars seeded, %zu forward bars restored, LC5thr+trail+window-flush, SHADOW, deploy-forward\n",
                   ilseeded, ilrestored);
            fflush(stdout);
        }
