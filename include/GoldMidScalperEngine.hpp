...
public:
    ...
    // S59 2026-05-07: PENDING-phase fill spread cap (2x MAX_SPREAD = 5.0).
    ...
    // 2026-05-05 (audit-fixes-43): Asia low-liquidity session gate.
    //   2026-05-05 05:08 UTC produced a LONG fire that SL_HIT in 5m6s for
    //   -$6.96. SL distance 6.68pt -- just above the post-S60 noise-floor
    //   minimum of 5.8pt -- still hit by routine Asia-session bid-ask
    //   oscillation. Same structural failure mode HBG had before
    //   audit-fixes-39 added its 06-22 UTC gate. MidScalperGold and
    //   HybridGold are sister engines with the same noise sensitivity in
    //   Asia hours; the fix is identical. 06-22 UTC retains London + NY
    //   + early Asia overlap (most legitimate compression breakouts fire
    //   there). Wraparound-aware ternary mirrors the EUR/AUD/NZD pattern
    //   from audit-fixes-37 -- when END > START it reduces to forward,
    ...
    // S61 2026-05-07: range_history persistence ctor (lineage source for FX cohort).
                 ...
                 bool   l2_real     = false) noexcept
    {
            ...
            if ((would_fill_long || would_fill_short) && spread > MAX_FILL_SPREAD) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[MID-SCALPER-GOLD] FILL_SPREAD_REJECT spread=%.2f max=%.2f side=%s -- aborting bracket\n",
        ...
        //   form matches the EUR/AUD/NZD pattern (and HBG's audit-fixes-39
        //   gate): when END > START it reduces to the forward case (utc_hour
        //   in [START..END)); otherwise the union of [START..23] and
        //   [0..END). For 06-22 it's the forward case.
        {
...
private:
...
#ifdef _WIN32
        return "C:\\Omega\\state\\gold_mid_scalper_range_history.csv";
#else
        return "state/gold_mid_scalper_range_history.csv";
#endif
    }
    ...
    void _try_load_range_history() noexcept {
        ...
        if (!f) {
            std::printf("[MID-SCALPER-GOLD] RANGE_HIST_LOAD no_file path=%s (cold-start)\n", path);
            std::fflush(stdout);
            return;
        }
        char line[128];
        ...
        if (saved_ts <= 0 || (now_s - saved_ts) > RANGE_HIST_STALENESS_S) {
            std::printf("[MID-SCALPER-GOLD] RANGE_HIST_LOAD stale saved=%lld age_s=%lld (cold-start)\n",
        ...
        std::printf("[MID-SCALPER-GOLD] RANGE_HIST_LOAD ok n=%d age_s=%lld\n",
    ...
    void _save_range_history() noexcept {
        ...
        if (!f) {
            std::printf("[MID-SCALPER-GOLD] RANGE_HIST_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
