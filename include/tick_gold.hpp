        ...
        double rtt_check)
{
    ...
    // ?? Outer gate diagnostic -- fires every 30s with EXACT block reason ????????
    {
    ...
    if (!g_candle_flow.has_open_position()
        ...
        // PATH A ONLY: require real M1 bars (bar_count >= 2).
        // PATH B (EMA proxy bar) and PATH C (synthetic drift bar) removed:
        // candle_is_bullish/bearish and breakout checks are meaningless on synthetic
        // bars and produced counter-trend entries. The 90s startup lockout already
        // covers the cold-start window before real bars arrive.
        ...
        // bar_count < 2 or m1_ready=false: cfe_bar.valid=false, entry block skipped.
        // CFE enters once the first real M1 bar completes (~60s after startup).
        ...
        if (cfe_bar.valid) {
            ...
            // Session-aware CFE risk cap (2026-04-13):
            // Asia (22:00-07:00 UTC): cap at 50% of configured risk.
            // At Asia ATR ~$2 with $80 risk: sl=1.4pt, size=0.57 lots -- too large.
            // At 50% ($40): sl=1.4pt, size=0.29 lots -- proportionate for thin tape.
            // London/NY: full configured risk applies.
            {
                const int64_t cfe_utc_sec  = now_ms_g / 1000LL;
                const int     cfe_utc_hour = static_cast<int>((cfe_utc_sec % 86400LL) / 3600LL);
                const bool    cfe_in_asia  = (cfe_utc_hour >= 22 || cfe_utc_hour < 7);
                const double  cfe_base_risk = (g_cfg.risk_per_trade_usd > 0.0)
                    ? g_cfg.risk_per_trade_usd : omega::CFE_RISK_DOLLARS;
                g_candle_flow.risk_dollars = cfe_in_asia
                    ? cfe_base_risk * 0.50
                    : cfe_base_risk;
            }
    ...
    if (!g_ema_cross.has_open_position()
        ...
        if (g_ema_cross.has_open_position()) {
            ...
            g_telemetry.UpdateLastSignal("XAUUSD",
                ...
                "ECE", regime.c_str(), "EMACross",
                g_ema_cross.pos.tp, g_ema_cross.pos.sl);
        }
    }
