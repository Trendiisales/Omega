...
//   CONSTRUCTION. neg=0 verified: full sweep, both WF halves, both regimes (uptrend USOILPos
//   + oil selloffs USOILNeg), real cost ~8bp. No separate loss-cut needed (a cold cut would
...
private:
    ...
    void close_leg_(int fi, int ti, bool up, double px, int64_t ts_sec, bool fwd, const char* reason) noexcept {
        ...
        if (fwd) {
            ...
            append_closed_(rec);
            std::printf("[USOIL][CLOSE] %s %s entry=%.4f exit=%.4f pts=%.4f (%s)\n",
