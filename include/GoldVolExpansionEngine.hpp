...
//
// LOG NAMESPACE: [GVX]. tr.engine = "GoldVolExpansion". tr.regime = "M5_VOL_EXPANSION".
// =============================================================================
...
public:
    ...
    double ATR_FLOOR     = 3.00;  // runtime tunable
    static constexpr double ATR_CAP       = 25.0;
    static constexpr double SPREAD_CAP    = 0.80;
    static constexpr int    MAX_HOLD_BARS = 24;     // 12h on M30
    static constexpr int    COOLDOWN_SEC  = 300;    // 5min
                 ...
                 const CloseCallback* ext_close = nullptr)
    {
        ...
        std::snprintf(buf, sizeof(buf),
            "[GVX] OPEN %s entry=%.2f sl=%.2f tp=%.2f size=%.3f atr=%.2f ema9=%.2f ema21=%.2f\n",
...
private:
    ...
    // Vol-expansion: rolling window of last 100 ATR values + recent N-bar range
    std::deque<double> m_atr_history;
    std::deque<double> m_recent_highs, m_recent_lows;
    static constexpr int VOL_WINDOW = 100;
    static constexpr int RANGE_WINDOW = 5;
    ...
    void _on_bar_close(const BarAccum& bar) {
        ...
        // Track ATR history for vol-rank computation
        if (m_atr.primed) {
            m_atr_history.push_back(m_atr.value);
            while ((int)m_atr_history.size() > VOL_WINDOW) m_atr_history.pop_front();
        }
        // Track recent range (high/low over last RANGE_WINDOW bars, EXCLUDING current)
        m_recent_highs.push_back(bar.high);
        m_recent_lows.push_back(bar.low);
        while ((int)m_recent_highs.size() > RANGE_WINDOW + 1) {
            m_recent_highs.pop_front();
            m_recent_lows.pop_front();
        }

        m_signal_pending=false;
        if (!m_atr.primed || !m_ema9.primed || !m_ema21.primed) return;
        if ((int)m_atr_history.size() < VOL_WINDOW) return;
        if ((int)m_recent_highs.size() <= RANGE_WINDOW) return;
        ...
        // VOL-EXPANSION ENTRY:
        //   1. ATR-rank: current ATR > 1.5x median of last 100 bars
        //      (vol is expanding -- directional commitment likely)
        //   2. Range-break: current close > prior 5-bar high (bull break)
        //      OR current close < prior 5-bar low (bear break)
        //   3. EMA alignment: bull break needs ema9>=ema21, bear needs <=
        //
        // Hypothesis: vol expansion = participants moving in size = directional
        // follow-through. Filters out chop-grinder false breakouts.

        // ATR median over last 100 bars (cheap: sort copy)
        std::vector<double> atrs(m_atr_history.begin(), m_atr_history.end());
        std::nth_element(atrs.begin(), atrs.begin() + atrs.size()/2, atrs.end());
        const double atr_median = atrs[atrs.size()/2];
        if (m_atr.value < 1.5 * atr_median) return;

        // Range over prior 5 bars (exclude current = .back())
        double rh = -1e18, rl = 1e18;
        for (size_t k = 0; k + 1 < m_recent_highs.size(); ++k) {
            if (m_recent_highs[k] > rh) rh = m_recent_highs[k];
            if (m_recent_lows[k]  < rl) rl = m_recent_lows[k];
        }
        const bool bull_break = (bar.close > rh);
        const bool bear_break = (bar.close < rl);
    ...
    void _close(double bid, double ask, int64_t now_ms, const char* reason, const CloseCallback* ext_close) {
        ...
        char buf[384];
        std::snprintf(buf, sizeof(buf),
            "[GVX] CLOSE %s entry=%.2f exit=%.2f pnl=$%.2f size=%.3f mfe=%.2f mae=%.2f be=%d bars=%d reason=%s\n",
        ...
        tr.engine="GoldVolExpansion"; tr.symbol="XAUUSD";
        tr.side=m_pos.is_long?"LONG":"SHORT"; tr.regime="M5_VOL_EXPANSION";
