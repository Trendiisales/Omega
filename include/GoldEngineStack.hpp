...
class MeanReversionEngine : public EngineBase {
    ...
    // Time-based dead zone removed. Gold trades 24h.
    // Spread + ATR quality gates protect against thin tape in all sessions.
    static bool in_dead_zone() noexcept { return false; }
...
// ?????????????????????????????????????????????????????????????????????????????
//
// Liquidity-pool sweep-reversal engine. Detects concentrated price clusters
// (liquidity pools) over a 120-tick window using detectLiqPool(): the densest
// cluster within a 0.35pt band requiring at least 8 prices in the cluster.
// When price extends >0.80pt from the densest cluster (SWEEP_TRIGGER) on a
// momentum spike >0.70pt (MOMENTUM_SPIKE) and that momentum is decaying
// (window-1 momentum < window-2 momentum * 0.60 == "exhausting" per
// momExhausting()), and price is also >2.00pt from VWAP (MIN_VWAP_DISTANCE),
// the engine fades back toward the pool. Direction is set by VWAP side:
// price above VWAP -> SHORT toward the pool below; price below VWAP -> LONG
// toward the pool above. A trend-alignment gate blocks fades into a live
// move (no SHORT into trend > 0.30, no LONG into trend < -0.30) so the
// engine only fires on exhausted momentum, not against active strength.
// Asia session has a tighter spread requirement (MAX_SPREAD * 0.55) to
// filter low-liquidity noise. Defaults: SL=18 ticks ($1.80), TP=2x SL
// (TP_RATIO is currently unused -- TP arms at 2x SL_TICKS in the signal).
//
