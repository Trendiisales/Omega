// =============================================================================
//  MinimalH4GER40Breakout.hpp  --  Pure H4 Donchian breakout engine for GER40
//
...
//      bar bundle with H4 OHLC + ATR14), there is no g_bars_ger40 in the codebase.
...
//      GER40 has no external H4 bar feed. This engine therefore builds its own
//      H4 OHLC bars and Wilder ATR14 internally from the live tick stream.
//
//  BACKTEST VALIDATION (Tickstory 2yr GER40, 21.5M ticks, 3,203 H4 bars):
...
//      See backtest/htf_bt_GER40_results.txt for full sweep.
//
...
//    7. Fixed $10 risk per trade, 0.10 lot cap (GER40 is $5/pt)
//
//  Exit:
...
//      deques + Wilder ATR state) to logs/bars_ger40_h4.dat. On startup we
...
//      One-shot bootstrap from Dukascopy CSV: see tools/seed_ger40_h4.cpp.
//      That tool replays Dukascopy GER40DAX H4 bars through the same Wilder/
//      Donchian math used here and writes a valid bars_ger40_h4.dat directly,
...
//    - $5/pt for GER40 means PnL math: pnl_dollars = pnl_pts * size * 500.0
//      (0.10 lot at $5/pt, scaled). Sizing: $10 / (sl_pts * 500) per lot.
//    - max_spread=3.0pt (GER40 has wider spreads than XAUUSD; gold uses 2.0).
...
//    unaffected. Gate is configurable via [minimal_h4_ger40] long_only=true in
...
// =============================================================================
//  Parameter struct -- GER40-specific defaults from 2yr Tickstory walk-forward
// =============================================================================
struct MinimalH4GER40Params {
...
// =============================================================================
//  MinimalH4GER40Breakout engine
// =============================================================================
struct MinimalH4GER40Breakout {
    ...
    struct OpenPos {
        ...
        double  size         = 0.0;
        double  h4_atr       = 0.0;
        double  mfe          = 0.0;  // S5 2026-05-06: max favourable excursion (price units, signed >=0). Was hardcoded to 0 at close.
        double  mae          = 0.0;  // S5 2026-05-06: max adverse excursion (price units, signed <=0). Was hardcoded to 0 at close.
    ...
    MinimalH4GER40Signal on_tick(double bid, double ask, int64_t now_ms,
                                CloseCallback on_close) noexcept
    {
        MinimalH4GER40Signal sig{};
        ...
        if (pos_.active) {
            // S5 2026-05-06: track MFE/MAE on every tick so the close-path emits
            // real values instead of hardcoded 0.0.
                ...
                && atr_pre > 0.0)
            {
                if (_is_weekend_gated(now_ms)) {
                    ...
                    if (bull_break || bear_break) {
                        ...
                        //   risk-off (macro OR price-based NAS sustained-bear fallback).
                        //   Donchian-breakout longs bleed in a bear; shorts unaffected.
                        ...
                        // GER40: $5/pt at 1.0 lot, so $0.05/pt at 0.01 lot.
...
private:
                ...
                int64_t now_ms, CloseCallback on_close) noexcept
    {
        ...
        // Example: 30pt move on 0.01 lot GER40 = 30 * 0.01 * 5.0 = $1.50.
        ...
        if (on_close) {
            ...
            tr.engine     = "MinimalH4GER40Breakout";
            tr.entryPrice = pos_.entry;
            tr.exitPrice  = exit_px;
            tr.sl         = pos_.sl;
            tr.size       = pos_.size;
            tr.pnl        = pnl_dollars;   // dollars, not points
            tr.mfe        = pos_.mfe * pos_.size;  // S5 2026-05-06: was hardcoded 0.0; now tracked per-tick
            tr.mae        = pos_.mae * pos_.size;  // S5 2026-05-06: was hardcoded 0.0; now tracked per-tick
            tr.entryTs    = pos_.entry_ts_ms / 1000;
            tr.exitTs     = now_ms / 1000;
            tr.exitReason = reason;
            tr.regime     = "MINIMAL_H4_GER40";
            tr.l2_live    = false;
