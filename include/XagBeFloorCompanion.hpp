#pragma once
// =============================================================================
// XagBeFloorCompanion — XAGPos (long) + XAGNeg (short) BE-floor clip books (SILVER).
//
// C++ in-binary engine, gold-twin of GoldBeFloorCompanion (byte-identical machinery,
// XAG params). Silver books in PRICE POINTS x $/pt (gold convention), NOT FX
...
//   XAGNeg 962 clips). $ magnitudes are FANTASY (a once-in-history silver parabola);
//   the real judge is the live shadow ledger + neg=0, same as every befloor.
//
...
//
// ADVERSE-PROTECTION: BE-FLOOR (no cut) — leg stays FLAT until price clears +be_bp from
//   ref (covers RT cost), opens there, stop sits >= entry (long) / <= entry (short) and
//   trails only favourably => exit >= entry ALWAYS => net_bp >= 0 every clip BY
//   CONSTRUCTION. neg=0 verified: full sweep, both WF halves, both regimes (bull XAGPos
//   + 2026 crash XAGNeg), real cost 6bp. No separate loss-cut needed (a cold cut would
//   only lower net — invalid here). Verdict = BE-floor (no cut), not skipped.
// =============================================================================
#include <cstdint>
#include <cstdio>
#include <cstring>
...
class XagBeFloorCompanion {
public:
    struct Config {
        int    W          = 2;        // detector window (H1 bars) -> "2h"
        double thr        = 0.010;    // detector threshold -> "+/-1%" (operator-LOCKED)
        double be_bp      = 6.0;      // XAG_RT_BP: real RT cost ~6bp (spread+comm+slippage)
        double lot        = 1.0;      // 1.0 std XAGUSD lot = 5000 oz
        double dpp_per_lot = 5000.0;  // $/point for 1.0 lot (silver lot = 5000 oz)
        std::string sym    = "XAGUSD";// symbol the live legs trade (order path)
        ...
        std::string state_path   = "xag_companion_state.json";       // served by /api/xag_companion
        std::string deploy_path  = "xag_companion_deploy_ts.txt";    // deploy-forward anchor (persisted)
        std::string book_path    = "xag_companion_book.txt";         // REAL forward book (persists across restarts)
        std::string live_path    = "xag_companion_live.txt";         // OPEN window+leg arm-state (persists across restarts)
        std::string closed_path  = "xag_companion_closed.csv";       // CLOSED forward trades list (persists; the "trades log")
        std::string dump_path    = "xag_companion_h1.csv";           // persisted LIVE H1 bars -> detector continuity across restart
                                                                     //   (XAG has no external H1 writer like gold_regime_h1.csv, so
    ...
    // ── LIVE EXECUTION WIRING — makes XAGPos/XAGNeg REAL independent trading engines ──
    ...
    explicit XagBeFloorCompanion(Config c) : cfg_(std::move(c)) {
    ...
    size_t seed_from_h1_csv(const std::string& rel) noexcept {
        ...
        std::printf("[XAGSIL][SEED] %zu H1 bars from %s (bars=%zu)\n", n, path.c_str(), ts_.size());
        std::fflush(stdout);
        return n;
    }
    ...
    void finalize_seed() noexcept {
        ...
        if (!deploy_loaded_ && !ts_.empty()) {
            ...
            std::printf("[XAGSIL][DEPLOY] deploy_ts stamped = %lld (forward-only book)\n",
                        (long long)deploy_ts_);
        } else {
            std::printf("[XAGSIL][DEPLOY] deploy_ts loaded = %lld (persisted)\n", (long long)deploy_ts_);
        }
        std::fflush(stdout);
    ...
    // Registered as the XAG H1 aggregator sink (on_tick.hpp) so it sees each completed H1. ----
...
private:
    ...
    static std::string LEG_ENGINE_(int fi, int ti) {
        return std::string(fi == 0 ? "XagBeFloorXAGPos" : "XagBeFloorXAGNeg") + ("_r" + std::to_string((long)LIVE_GB_[ti]));
    }
                    ...
                    if (cond) {
                        ...
                        if (viable) {
                            ...
                            if (fwd) {
                                ...
                                std::printf("[XAGSIL][OPEN] %s %s @%.4f lot=%.2f tok=%s\n",
    ...
    void close_leg_(int fi, int ti, bool up, double px, int64_t ts_sec, bool fwd, const char* reason) noexcept {
        ...
        if (fwd) {
            ...
            // REAL forward book: raw price-pts booked. BE-floor guarantees p>=0 -> neg=0 holds live.
            ...
            append_closed_(rec);
            std::printf("[XAGSIL][CLOSE] %s %s entry=%.4f exit=%.4f pts=%.4f (%s)\n",
    ...
    std::string build_state_() const {
        ...
        for (int fi = 0; fi < 2; ++fi) {
            ...
            for (int ti = 0; ti < NT_; ++ti) {
                ...
                runs.precision(4); runs << "\"pts\":" << b.pts << ",";
            ...
            fl.precision(4); fl << "\"book_pts\":" << fwd_pts << ",";
        ...
        o << "\"bars\":" << ts_.size() << ",\"shadow\":true,"
          << "\"engine\":\"xag-befloor-XAGPos-XAGNeg\",\"deploy_ts\":" << (long long)deploy_ts_ << ",";
        o.precision(4); o << "\"desk_pts\":" << desk_pts << ",";
...
inline XagBeFloorCompanion& xag_befloor_companion() noexcept {
    static XagBeFloorCompanion inst = []{ return XagBeFloorCompanion(XagBeFloorCompanion::Config{}); }();
    return inst;
}
