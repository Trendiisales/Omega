...
public:
    struct Config {
        ...
        double rt_cost_bp = 6.0;      // REAL round-trip cost (bp of entry) debited from every clip's
            ...
            // never re-armed. W>1 usually self-heals on the next live bar; this closes the seam.
            {
                ...
                if (Nsa > Wsa) {
                    ...
                    if (win_pend_[0] || win_pend_[1]) {
                        save_live_state_();
                        std::printf("[XAGSIL][SEED-ARM] %s last seed bar %+.3f%% -> pending %s carried; next live bar opens window\n",
...
private:
    ...
    void close_leg_(int fi, int ti, bool up, double px_floor, double px_obs,
        ...
        if (fwd) {
            ...
            append_closed_(rec);
            std::printf("[XAGSIL][CLOSE] %s %s entry=%.4f fill=%.4f pts_model=%.4f pts_real=%.4f (%s)\n",
