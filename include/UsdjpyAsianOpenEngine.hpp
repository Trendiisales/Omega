...
public:
    ...
    static constexpr double MAX_FILL_SPREAD      = 0.04;
    ...
    //   USDJPY uses 0.01 = 1 pip scale (JPY-quote pair). Floor = 3 pips
    //   = 0.03 in raw price units. Range must clear median by at least
    //   ABS_EXPANSION_FLOOR in addition to the 1.10x multiplier. Blocks
    //   marginal-clearance fires.
    static constexpr double ABS_EXPANSION_FLOOR   = 0.03;

    // S64 2026-05-13 circuit-breaker layer (mirrors GbpusdLondonOpen,
    //   but in JPY price units: 0.03% of 154.0 = 0.0462 ~= 4.6 pips).
    //   The PCT-of-entry math is unit-agnostic; only the absolute price
    //   format strings differ (%.3f vs %.5f) in the log lines.
    ...
    //   safe fallback.
    UsdjpyAsianOpenEngine() noexcept {
                 ...
                 bool   l2_real     = false) noexcept
    {
            ...
            if ((would_fill_long || would_fill_short) && spread > MAX_FILL_SPREAD) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[JPY-ASN-OPEN] FILL_SPREAD_REJECT spread=%.3f max=%.3f side=%s -- aborting bracket\n",
        ...
        if (phase == Phase::IDLE) {
            // S64 2026-05-13 consec-loss circuit breaker.
            {
                    ...
                    m_consec_loss_day_utc != -1)
                {
                        ...
                        m_consec_loss_block_until_s > 0)
                    {
                        char _buf[256];
                        snprintf(_buf, sizeof(_buf),
                            "[JPY-ASN-OPEN] CONSEC_LOSS_DAY_ROLL reset count=%d->0 "
                            "block_until=%lld->0\n",
                            m_consec_loss_count,
                ...
                now_s < m_consec_loss_block_until_s)
            {
                ...
                if (now_s - s_last_log_s >= 60) {
                    ...
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[JPY-ASN-OPEN] CONSEC_LOSS_BLOCK active count=%d "
            ...
            if (m_sl_price > 0.0 && now_s < m_sl_cooldown_ts) {
                if (std::fabs(w_hi - m_sl_price) < SAME_LEVEL_BLOCK_PTS ||
                    ...
                    //   JPY format uses %.3f (raw price like 154.123).
                    {
                        char _buf[320];
                        snprintf(_buf, sizeof(_buf),
                            "[JPY-ASN-OPEN] SAME_LEVEL_BLOCK side=SL sl_price=%.3f "
                            "w_hi=%.3f w_lo=%.3f radius=%.3f remaining_s=%lld\n",
            ...
            if (m_win_exit_price > 0.0 && now_s < m_win_exit_block_ts) {
                if (std::fabs(w_hi - m_win_exit_price) < SAME_LEVEL_BLOCK_PTS ||
                    std::fabs(w_lo - m_win_exit_price) < SAME_LEVEL_BLOCK_PTS) {
                    {
                        char _buf[320];
                        snprintf(_buf, sizeof(_buf),
                            "[JPY-ASN-OPEN] SAME_LEVEL_BLOCK side=WIN win_price=%.3f "
                            "w_hi=%.3f w_lo=%.3f radius=%.3f remaining_s=%lld\n",
        ...
        if (phase == Phase::ARMED) {
            ...
            if ((int)m_range_history.size() < EXPANSION_MIN_HISTORY) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[JPY-ASN-OPEN] COLD_START_BLOCK range=%.3f hist=%d/%d -- skipping fire\n",
...

            {
                ...
                //   floor (median + 3 pips = median + 0.03). JPY pip scale
                //   is 0.01 = 1 pip so the absolute floor is 0.03.
                ...
                if (range < pct_threshold || range < abs_threshold) {
                    {
                        char _buf[384];
                        snprintf(_buf, sizeof(_buf),
                            "[JPY-ASN-OPEN] ATR_GATE_FAIL range=%.3f median=%.3f "
                            "mult=%.2f pct_thr=%.3f abs_floor=%.3f abs_thr=%.3f hist=%d\n",
            ...
            if (!ExecutionCostGuard::is_viable(
                    "USDJPY", spread, tp_dist, base_lot, 1.5))
            {
                phase = Phase::IDLE;
                bracket_high = bracket_low = range = 0.0;
                return;
                ...
                int64_t now_s, CloseCallback on_close) noexcept
    {
        ...
        //   JPY price format: %.3f (e.g. 154.123). PCT math is unit-agnostic.
        if (LOSS_CUT_PCT > 0.0 && pos.entry > 0.0) {
            ...
            if (adverse >= loss_cut_dist) {
                const double exit_px = pos.is_long ? bid : ask;
                {
                    char _buf[320];
                    snprintf(_buf, sizeof(_buf),
                        "[JPY-ASN-OPEN] LOSS_CUT adverse=%.3f >= %.3f "
                        "(%.3f%% of entry %.3f) -- cutting immediately\n",
...
private:
...
#ifdef _WIN32
        return "C:\\Omega\\state\\usdjpy_asian_open_range_history.csv";
#else
        return "state/usdjpy_asian_open_range_history.csv";
#endif
    }
    ...
    //   JPY price scale is 0.01 = 1 pip; save uses %.10f the same as FX
    //   majors -- standard double precision in textual format.
    static const char* _post_trade_block_path() noexcept {
#ifdef _WIN32
        return "C:\\Omega\\state\\usdjpy_asian_open_post_trade_block.csv";
#else
        return "state/usdjpy_asian_open_post_trade_block.csv";
#endif
    }
    ...
    void _try_load_post_trade_block() noexcept {
        ...
        if (!f) {
            std::printf("[JPY-ASN-OPEN] POST_TRADE_BLOCK_LOAD no_file path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        char line[256];
        ...
        std::printf("[JPY-ASN-OPEN] POST_TRADE_BLOCK_LOAD restored=%d sl_price=%.3f sl_rem_s=%lld win_price=%.3f win_rem_s=%lld "
    ...
    void _save_post_trade_block() noexcept {
        ...
        if (!f) {
            std::printf("[JPY-ASN-OPEN] POST_TRADE_BLOCK_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
    ...
    void _try_load_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "r");
        if (!f) {
            std::printf("[JPY-ASN-OPEN] RANGE_HIST_LOAD no_file path=%s (cold-start)\n", path);
            std::fflush(stdout);
            return;
        }
        char line[128];
        ...
        if (saved_ts <= 0 || (now_s - saved_ts) > RANGE_HIST_STALENESS_S) {
            std::printf("[JPY-ASN-OPEN] RANGE_HIST_LOAD stale saved=%lld age_s=%lld (cold-start)\n",
        ...
        std::printf("[JPY-ASN-OPEN] RANGE_HIST_LOAD ok n=%d age_s=%lld\n",
    ...
    void _save_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "w");
        if (!f) {
            std::printf("[JPY-ASN-OPEN] RANGE_HIST_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
                ...
                int64_t now_s, CloseCallback on_close) noexcept
    {
        ...
        if (_is_loss_exit) {
                ...
                m_consec_loss_count >= CONSEC_LOSS_THRESH)
            {
                ...
                char _buf[384];
                snprintf(_buf, sizeof(_buf),
                    "[JPY-ASN-OPEN] CONSEC_LOSS_BLOCK_TRIPPED count=%d "
