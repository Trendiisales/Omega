                 ...
                 bool   l2_real     = false) noexcept
    {
            ...
            if ((would_fill_long || would_fill_short) && spread > MAX_FILL_SPREAD) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[NZD-ASN-OPEN] FILL_SPREAD_REJECT spread=%.5f max=%.5f side=%s -- aborting bracket\n",
        ...
        if (phase == Phase::IDLE) {
            // S64 2026-05-13 consec-loss circuit breaker.
            {
                    ...
                    m_consec_loss_day_utc != -1)
                {
                        ...
                        m_consec_loss_block_until_s > 0)
                    {
                        char _buf[256];
                        snprintf(_buf, sizeof(_buf),
                            "[NZD-ASN-OPEN] CONSEC_LOSS_DAY_ROLL reset count=%d->0 "
                            "block_until=%lld->0\n",
                            m_consec_loss_count,
                ...
                now_s < m_consec_loss_block_until_s)
            {
                ...
                if (now_s - s_last_log_s >= 60) {
                    ...
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[NZD-ASN-OPEN] CONSEC_LOSS_BLOCK active count=%d "
        ...
        if (phase == Phase::ARMED) {
            ...
            if ((int)m_range_history.size() < EXPANSION_MIN_HISTORY) {
                {
                    char _buf[256];
                    snprintf(_buf, sizeof(_buf),
                        "[NZD-ASN-OPEN] COLD_START_BLOCK range=%.5f hist=%d/%d -- skipping fire\n",
            ...
            if (!ExecutionCostGuard::is_viable(
                    "NZDUSD", spread, tp_dist, base_lot, 1.5))
            {
                phase = Phase::IDLE;
                bracket_high = bracket_low = range = 0.0;
                return;
        ...
        if (LOSS_CUT_PCT > 0.0 && pos.entry > 0.0) {
            ...
            if (adverse >= loss_cut_dist) {
                const double exit_px = pos.is_long ? bid : ask;
                {
                    char _buf[320];
                    snprintf(_buf, sizeof(_buf),
                        "[NZD-ASN-OPEN] LOSS_CUT adverse=%.5f >= %.5f "
...
private:
    ...
    void _try_load_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "r");
        if (!f) {
            std::printf("[NZD-ASN-OPEN] RANGE_HIST_LOAD no_file path=%s (cold-start)\n", path);
            std::fflush(stdout);
            return;
        }
        char line[128];
        ...
        if (saved_ts <= 0 || (now_s - saved_ts) > RANGE_HIST_STALENESS_S) {
            std::printf("[NZD-ASN-OPEN] RANGE_HIST_LOAD stale saved=%lld age_s=%lld (cold-start)\n",
        ...
        std::printf("[NZD-ASN-OPEN] RANGE_HIST_LOAD ok n=%d age_s=%lld\n",
    ...
    void _save_range_history() noexcept {
        ...
        FILE* f = std::fopen(path, "w");
        if (!f) {
            std::printf("[NZD-ASN-OPEN] RANGE_HIST_SAVE_FAIL path=%s\n", path);
            std::fflush(stdout);
            return;
        }
        const long long now_s = (long long)std::time(nullptr);
                ...
                int64_t now_s, CloseCallback on_close) noexcept
    {
        ...
        if (_is_loss_exit) {
                ...
                m_consec_loss_count >= CONSEC_LOSS_THRESH)
            {
                ...
                char _buf[384];
                snprintf(_buf, sizeof(_buf),
                    "[NZD-ASN-OPEN] CONSEC_LOSS_BLOCK_TRIPPED count=%d "
