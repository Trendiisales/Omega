    ...
    <style>
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        header { background:var(--glass); border:1px solid var(--border); border-radius:12px; padding:10px 20px;
            ...
            box-shadow:0 4px 24px rgba(0,0,0,0.4); gap:16px; }
        ...
        /* ── HEADER TICKER STRIP — Gold + Silver prominent in the middle ── */
        .hdr-tickers { display:flex; align-items:center; gap:6px; flex:1; justify-content:center; flex-wrap:nowrap; overflow:hidden; }
        .hdr-ticker { display:flex; align-items:center; gap:5px; padding:5px 10px; border-radius:8px;
            border:1px solid var(--border); background:rgba(255,255,255,0.03); white-space:nowrap; }
        .hdr-ticker.gold-ticker { border-color:rgba(240,192,64,0.35); background:rgba(240,192,64,0.06); }
        .hdr-ticker.silver-ticker { border-color:rgba(176,196,216,0.35); background:rgba(176,196,216,0.06); }
        .hdr-ticker-sym { font-size:9px; font-weight:700; letter-spacing:1px; color:var(--t2); text-transform:uppercase; }
        .hdr-ticker.gold-ticker .hdr-ticker-sym   { color:var(--gold); }
        .hdr-ticker.silver-ticker .hdr-ticker-sym { color:var(--silver); }
        .hdr-ticker-bid { font-family:'Space Mono',monospace; font-size:13px; font-weight:700; color:var(--green); }
        .hdr-ticker-ask { font-family:'Space Mono',monospace; font-size:13px; font-weight:700; color:var(--red); }
        .hdr-ticker-spread { font-family:'Space Mono',monospace; font-size:9px; color:var(--t3); }
        .hdr-ticker-sep { color:var(--t3); font-size:10px; margin:0 2px; }
        .hdr-ticker-phase { font-size:8px; font-weight:700; letter-spacing:0.5px; padding:1px 5px;
            border-radius:3px; margin-left:4px; }
        .phase-flat   { background:rgba(255,255,255,0.06); color:var(--t2); }
        .phase-comp   { background:rgba(255,149,0,0.15);   color:var(--amber); }
        .phase-brk    { background:rgba(0,232,122,0.18);   color:var(--green); animation:brk-pulse 0.8s ease-in-out infinite; }
        @keyframes brk-pulse { 0%,100%{opacity:1;} 50%{opacity:0.6;} }

        .hbar { display:flex; align-items:center; gap:8px; flex-shrink:0; flex-wrap:nowrap; }
        ...
        /* ── LEFT COLUMN — symbols ordered by trade frequency ── */
        ...
        .sym-row.gold-sym    { border-color:rgba(240,192,64,0.22); background:rgba(240,192,64,0.05); }
        .sym-row.silver-sym  { border-color:rgba(176,196,216,0.22); background:rgba(176,196,216,0.05); }
        .sym-label { font-weight:700; font-size:11px; color:var(--t2); letter-spacing:1px; min-width:60px; }
        .sym-label.primary { color:var(--blue); }
        .sym-label.gold    { color:var(--gold); }
        .sym-label.silver  { color:var(--silver); }
...
<div class="desk">
    <header>
        ...
        <!-- ── GOLD + SILVER TICKER STRIP — centre of header ── -->
        <div class="hdr-tickers">
            <!-- GOLD — highest trade frequency -->
            <div class="hdr-ticker gold-ticker">
                <span class="hdr-ticker-sym">GOLD</span>
                <span class="hdr-ticker-bid" id="hdrGoldBid">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrGoldAsk">----</span>
                <span class="hdr-ticker-spread" id="hdrGoldSpread">--</span>
                <span class="hdr-ticker-phase phase-flat" id="hdrGoldPhase">--</span>
            </div>
            <!-- SILVER — second highest frequency -->
            <div class="hdr-ticker silver-ticker">
                <span class="hdr-ticker-sym">SILVER</span>
                <span class="hdr-ticker-bid" id="hdrXagBid">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrXagAsk">----</span>
                <span class="hdr-ticker-spread" id="hdrXagSpread">--</span>
                <span class="hdr-ticker-phase phase-flat" id="hdrXagPhase">--</span>
            </div>
            <!-- Divider -->
            <div style="width:1px;height:28px;background:var(--border);margin:0 4px;"></div>
            <!-- Other actives — compact -->
            <div class="hdr-ticker">
                <span class="hdr-ticker-sym">US500</span>
                <span class="hdr-ticker-bid" id="hdrSpBid" style="font-size:11px">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrSpAsk" style="font-size:11px">----</span>
            </div>
            <div class="hdr-ticker">
                <span class="hdr-ticker-sym">USTEC</span>
                <span class="hdr-ticker-bid" id="hdrNqBid" style="font-size:11px">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrNqAsk" style="font-size:11px">----</span>
            </div>
            <div class="hdr-ticker">
                <span class="hdr-ticker-sym">OIL</span>
                <span class="hdr-ticker-bid" id="hdrClBid" style="font-size:11px">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrClAsk" style="font-size:11px">----</span>
            </div>
            <div class="hdr-ticker">
                <span class="hdr-ticker-sym">BRENT</span>
                <span class="hdr-ticker-bid" id="hdrBrentBid" style="font-size:11px">----</span>
                <span class="hdr-ticker-sep">|</span>
                <span class="hdr-ticker-ask" id="hdrBrentAsk" style="font-size:11px">----</span>
            </div>
            <div class="hdr-ticker">
                <span class="hdr-ticker-sym">EURUSD</span>
                <span class="hdr-ticker-bid" id="hdrEurBid" style="font-size:11px">----</span>
                <span class="hdr-ticker-sep">|</span>
    ...
    <div class="main">
        <!-- LEFT COLUMN — ordered by trade frequency -->
        <div class="col-left">
            <div class="card">
                <div class="card-hd"><span class="dot"></span>Market Data</div>

                <!-- 1. GOLD — highest frequency -->
                <div class="sym-group-label">★ Most Active</div>
                <div class="sym-row gold-sym">
                    <span class="sym-label gold">GOLD.F</span>
                    <div class="px-pair"><span class="bid" id="goldBid">----</span><span style="color:var(--t3)">|</span><span class="ask" id="goldAsk">----</span></div>
                    <span class="sym-spread" id="goldSpread">--</span>
                </div>
                <!-- 2. SILVER -->
                <div class="sym-row silver-sym">
                    <span class="sym-label silver">XAGUSD</span>
                    <div class="px-pair"><span class="bid" id="xagBid">----</span><span style="color:var(--t3)">|</span><span class="ask" id="xagAsk">----</span></div>
                    <span class="sym-spread" id="xagSpread">--</span>
                </div>

                <!-- 3-7: Indices + Oil — secondary trading -->
                <div class="sym-group-label" style="margin-top:8px;">▶ Trading</div>
                ...
                <div class="sym-row primary-sym">
                    <span class="sym-label primary">UKBRENT</span>
                    <div class="px-pair"><span class="bid" id="brentBid">----</span><span style="color:var(--t3)">|</span><span class="ask" id="brentAsk">----</span></div>
                    <span class="sym-spread" id="brentSpread">--</span>
                </div>
                <div class="sym-row primary-sym">
                ...
                <div class="sym-row">
                    <span class="sym-label">GER30</span>
                    <div class="px-pair"><span class="bid" id="ger30Bid" style="font-size:11px">--</span><span style="color:var(--t3)">|</span><span class="ask" id="ger30Ask" style="font-size:11px">--</span></div>
                </div>
                <div class="sym-row">
                    <span class="sym-label">UK100</span>
                    <div class="px-pair"><span class="bid" id="uk100Bid" style="font-size:11px">--</span><span style="color:var(--t3)">|</span><span class="ask" id="uk100Ask" style="font-size:11px">--</span></div>
                </div>
                <div class="sym-row">
...
R"OMEGA2(
        ...
        <div class="bottom-row">
            ...
            <div class="card">
                ...
                <div id="sysLog" style="font-size:10px;color:var(--t2);font-family:'Space Mono',monospace;max-height:80px;overflow-y:auto;">Ready.</div>
            </div>
        </div>
    </div>
</div>
...
function toggleBell(){
    ...
    if(b){
        if(_bellEnabled){b.textContent='🔔 BELL: ARMED';b.style.color='var(--green)';b.style.borderColor='var(--green)';b.style.background='rgba(0,230,118,.1)';}
        else{b.textContent='🔔 ARM BELL';b.style.color='#ffd600';b.style.borderColor='#ffd600';b.style.background='rgba(255,214,0,.1)';}
    }
...
function setPrice(id,val,dec){const el=document.getElementById(id);if(!el)return;const v=safe(val);if(v>0)el.textContent=v.toFixed(dec||2);}
function setSpread(id,bid,ask){const el=document.getElementById(id);if(!el)return;const b=safe(bid),a=safe(ask);if(b>0&&a>0)el.textContent=(a-b).toFixed(2);}

// Header ticker phase badge
function setHdrPhase(id,phase){
    const el=document.getElementById(id);if(!el)return;
    const p=safe(phase);
    if(p===0){el.className='hdr-ticker-phase phase-flat';el.textContent='FLAT';}
    else if(p===1){el.className='hdr-ticker-phase phase-comp';el.textContent='COMP';}
    else{el.className='hdr-ticker-phase phase-brk';el.textContent='BRK ⚡';}
}

function updateEngineCard(prefix,phase,compHi,compLo,rv,bv,sigs){
    const card=document.getElementById('eng'+prefix),badge=document.getElementById('eng'+prefix+'Phase'),vol=document.getElementById('eng'+prefix+'Vol'),sig=document.getElementById('eng'+prefix+'Sig');
    if(!card)return;const p=safe(phase);card.className='eng-state-card phase-'+p;
    ...
    if(p===1&&safe(compHi)>0){if(vol)vol.textContent='Hi='+safe(compHi).toFixed(2)+' Lo='+safe(compLo).toFixed(2);}
    else{if(vol)vol.textContent='rv '+safe(rv).toFixed(3)+'% bv '+safe(bv).toFixed(3)+'%';}
...
function renderCompRanges(d){
    const el=document.getElementById('compRanges');if(!el)return;
    const syms=[
    ...
    syms.forEach(s=>{
        const p=safe(s.phase),col=p===1?'var(--amber)':p===2?'var(--green)':'var(--t3)',phase=p===0?'FLAT':p===1?'COMPRESSING':'BREAKOUT';
        html+='<div style="text-align:center;">'
            ...
            +(p===1&&safe(s.hi)>0?'<div style="font-family:Space Mono,monospace;font-size:9px;color:var(--t2);margin-top:2px;">'+safe(s.hi).toFixed(2)+' ↑ '+safe(s.lo).toFixed(2)+'</div>'
...
function renderTrades(trades){
    ...
    if(_bellEnabled&&closed.length>_lastTradeCount&&_lastTradeCount>=_bellBootCount){const pnl=safe(closed[0].net_pnl);if(pnl>0){_playWinBell();}else{_playLossBell();}}
    ...
    if(cE)cE.textContent=closed.length+' closed · '+wins+'W/'+losses+'L · '+(totalNet>=0?'+':'')+  '$'+Math.abs(totalNet).toFixed(2)+' net';
    const now=Math.floor(Date.now()/1000);
    el.innerHTML=trades.slice(0,50).map(t=>{
        const isOpen=!t.exitReason||t.exitReason==='',netPnl=safe(t.net_pnl),grossPnl=safe(t.pnl),slip=safe(t.slippage_entry)+safe(t.slippage_exit);
        ...
        const pnlC=isWin?'var(--green)':isLoss?'var(--red)':'var(--t2)',sideC=t.side==='LONG'?'var(--green)':'var(--red)';
        const reason=t.exitReason||'',result=isOpen?'🔵 OPEN':reason==='TP_HIT'?'✅ TP':reason==='SL_HIT'?'❌ SL':reason==='TIMEOUT'?'⏱ TO':'⚡ FC';
        const resultC=isOpen?'var(--blue)':reason==='TP_HIT'?'var(--green)':reason==='SL_HIT'?'var(--red)':'var(--t2)';
        let heldStr='--';
...
function updateDashboard(d){
    ...
    // Header tickers — Gold + Silver prominent
    setPrice('hdrGoldBid',d.gold_bid,2);setPrice('hdrGoldAsk',d.gold_ask,2);setSpread('hdrGoldSpread',d.gold_bid,d.gold_ask);
    setPrice('hdrXagBid', d.xag_bid, 3);setPrice('hdrXagAsk', d.xag_ask, 3);setSpread('hdrXagSpread', d.xag_bid, d.xag_ask);
    setHdrPhase('hdrGoldPhase', d.xau_phase);
    setPrice('hdrSpBid',  d.sp_bid,   2);setPrice('hdrSpAsk',  d.sp_ask,   2);
    setPrice('hdrNqBid',  d.nq_bid,   2);setPrice('hdrNqAsk',  d.nq_ask,   2);
...
R"OMEGA4(
    ...
    setPrice('uk100Bid',d.uk100_bid,2);setPrice('uk100Ask',d.uk100_ask,2);
    setPrice('estx50Bid',d.estx50_bid,2);setPrice('estx50Ask',d.estx50_ask,2);
    ...
    // PnL
    const pnl=safe(d.daily_pnl),grossPnl=safe(d.gross_daily_pnl);
    const pE=document.getElementById('pnlVal');
    if(pE){pE.textContent=(pnl>=0?'+':'')+'$'+Math.abs(pnl).toFixed(2);pE.className='pnl-val '+(pnl>=0?'pnl-pos':'pnl-neg');}
    document.getElementById('pnlSub').textContent=safe(d.total_trades)+' trades · '+safe(d.win_rate).toFixed(1)+'% win';
    const gSub=document.getElementById('pnlGrossSub');
    if(gSub&&grossPnl!==0){const slip=Math.abs(grossPnl-pnl);gSub.textContent='gross '+(grossPnl>=0?'+':'')+'$'+Math.abs(grossPnl).toFixed(2)+' · slip -$'+slip.toFixed(2);}
    ...
    document.getElementById('msgRate').textContent=safe(d.quote_msg_rate)||'--';
    ...
    if(sb){const now=new Date(),mins=now.getUTCHours()*60+now.getUTCMinutes();let sess,col;
...
function setConn(ok){const d=document.getElementById('connDot'),t=document.getElementById('connText');if(d)d.className='conn-dot '+(ok?'ok':'bad');if(t)t.textContent=ok?(wsConnected?'Live (WS)':'Connected'):'Reconnecting...';}
...
function connectWS(){
    ...
    ws.onclose=()=>{wsConnected=false;setConn(false);if(wsFailCount>=5){wsGiveUp=true;return;}const delay=Math.min(2000*Math.pow(2,wsFailCount),15000);setTimeout(connectWS,delay);};
}
