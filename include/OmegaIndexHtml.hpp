...
/* S-2026-07-11: CSS multicol (columns:2) could NOT balance the tall unsplittable cstacks -- it
   dumped both into the left column and left the right column EMPTY (operator's wasted space).
   Switched to FLEX: the direct children (the 2 .cstack column-groups) sit side by side as two
   real columns -> no empty column is possible. */
.compcols{display:flex;gap:6px;align-items:flex-start;margin-top:6px}
.compcols>.pan{flex:1;min-width:0;margin:0}
...
#alltrades td,#alltrades th{padding:0 6px;font-size:9px;line-height:1.3}  /* S-2026-07-10 operator: compact LAST 15 TRADES rows (were spread out / wasted space) */
...
/* neon TRADING beacon (operator 2026-07-18: "much brighter neon green symbol when it is trading") */
...
<div class="pan" style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;background:var(--pan2);padding:7px 12px;border-radius:6px 6px 0 0">
  ...
  <!-- per-asset-class split of ALL-TIME (STOCK+CRYPTO+FX+GOLD == ALL-TIME; paper/forward books folded in by class). Wraps on a narrow bar. -->
  <span id="clspnl" style="display:inline-flex;align-items:baseline;gap:9px;flex-wrap:wrap" title="per-asset-class breakdown of ALL-TIME — STOCK + CRYPTO + FX + GOLD sum to ALL-TIME. CRYPTO = REAL Binance cash only (S-18w); crypto paper banks shown in panels, never here.">
    <span style="display:inline-flex;align-items:baseline;gap:4px"><span class="lbl">STOCK</span><span id="pnl_stk" class="num" style="font-size:12px;font-weight:600;color:var(--t2)">…</span></span>
    <span style="display:inline-flex;align-items:baseline;gap:4px"><span class="lbl">CRYPTO</span><span id="pnl_cry" class="num" style="font-size:12px;font-weight:600;color:var(--t2)">…</span></span>
    <span style="display:inline-flex;align-items:baseline;gap:4px"><span class="lbl">FX</span><span id="pnl_fx" class="num" style="font-size:12px;font-weight:600;color:var(--t2)">…</span></span>
  ...
  <span class="lbl"><span class="dot" id="trdot" style="background:var(--t3);width:10px;height:10px"></span><span id="trdlbl" style="font-weight:700;letter-spacing:.5px">IDLE</span></span>
  <!-- CC truth chip (S-2026-07-18af, operator: "see IMMEDIATELY correct software loaded"): chimera
       executor verified state — Mac watch probes josgp1 every 1min, relay pushes /api/chimera_health.
       GREEN only when build==repo HEAD && mode LIVE && no restart-loop && config consistent && feed
       fresh. STALE/RED = NOT verified — do not trust that the crypto engine is trading. -->
  <span class="lbl" title="chimera (josgp1 crypto executor) verified-truth chip: build==repo HEAD, RUNTIME MODE LIVE, no restart-loop (>3 starts/30min), no config mode-conflict, watch+relay fresh. RED/STALE = executor NOT verified live — check chimera-direct."><span class="dot" id="ccdot" style="background:var(--t3);width:10px;height:10px"></span><span id="cclbl" style="font-weight:700;letter-spacing:.5px">CC …</span></span>
...
<div style="border:1px solid var(--bd);border-top:none;border-radius:0 0 6px 6px;background:var(--pan);padding:7px 12px;margin-top:-6px">
  ...
  <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:5px">
    ...
    <span id="goldreg" class="chip" title="gold PRICE-regime gate. Long-only gold engines warm-seed + ARM on boot; when quiet they are HELD by this regime (BEAR/MACRO-HOSTILE), never cold-warming. Flat gold => EMA200 not falling => NEUTRAL => longs auto-unblock." style="background:var(--pan2);color:var(--t2)">…</span>
...
<!-- ═══ WHOLE-DESK TWO-COLUMN LAYOUT (S-2026-07-22h, operator: "gaping space" screenshot) ═══
     The old .topgrid row (chart | engine ledger) and the .compcols row below it were SEPARATE
     grid rows: after S-22g moved HEAT+TRADES out, the right topgrid cell held only the short
     ENGINE LEDGER while the row stretched to the chart's height — a chart-sized void. Fix is
     structural, not spacer tuning: ONE .compcols flex holding two continuously-flowing .cstack
     columns, so panels always pack upward and no cross-row barrier can strand empty space.
     LEFT  = LIVE TRADES → PREDICTIVE RANGES → CRYPTO MIMIC → BIGCAP HI52 → ENGINE HEAT → LAST 15
     RIGHT = ENGINE LEDGER → STOCK MOVERS → STOCK BASKET → INDEX LADDER → FX LADDER -->
...
<!-- ═══ MONEY RAIL removed 2026-07-06 (operator: PRE-TRADE EQUITY + TRADE HISTORY useless — paper diagnostics).
     deskmain now single-column: the price chart spans full width. ═══ -->

</div><!-- /deskmain -->

<!-- ENGINE LEDGER moved to the TOP of the RIGHT cstack (S-2026-07-22h) — the left column
     continues directly below with CRYPTO MIMIC → BIGCAP HI52 → ENGINE HEAT → LAST 15 TRADES. -->
...
<div class="pan">
  ...
  <!-- OVERALL live PnL at the TOP of the box (operator S-2026-07-19: overall PnL up top,
       then each mimic gets its own PnL line under the parent trade it mimics). -->
  <div id="ccrtotal" style="font-size:15px;font-weight:700;margin-bottom:6px">…</div>
  <div style="overflow:auto;max-height:340px"><table id="ccrtab"><tr><td class="l d">loading…</td></tr></table></div>
</div>

<!-- CRYPTO MIMIC BOOKS panel (pre-trade version) REMOVED S-2026-07-19g (operator: "if its real
...
<!-- ═══ ENGINE LEDGER (running totals) — moved here S-2026-07-22h from the old lone topgrid
     ...
     PROMOTION TRACKER removed 2026-07-06 (operator: useless). ═══ -->
<div class="pan">
...
<!-- ═══ STOCK MOVERS — BIGCAP day-mover MIMIC LADDER books (no-floor · pre-trade, additive) ═══ -->
<div class="pan">
  <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:6px">
    <span class="lbl">STOCK MOVERS — BIGCAP mimic LADDER (no-floor) · +3% day → next-close entry · TIGHT a0.5/s2 + WIDE a8/g50 + ladder cap5 · LOSS_CUT 15 · BT +7,044% PF1.58 all-6 (native C++ · pre-trade · REAL fills · never vs-WIDE)</span>
...
<!-- ═══ INDEX LADDER — H1 mimic giveback LADDER books (BE-floored-on-open · pre-trade, additive · S-2026-07-17 1ba35842) ═══ -->
<div class="pan">
  <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap;margin-bottom:6px">
    <span class="lbl">INDEX LADDER — H1 mimic LADDER (BE-floored-on-open, unrealized mark can still show red pre-exit) · US500 W24/2.0 · NAS100 W24/1.5 · GER40 W12/1.5 BULL-GATED (index_risk_off) · TIGHT+WIDE+STACKED + reclip cap5 · LC 5×thr floored (native C++ · pre-trade · never vs-WIDE)</span>
  ...
  <div id="ixladopenwrap" style="display:none;margin-top:6px"><div class="lbl" style="color:var(--grn)">OPEN NOW (live legs)</div><div style="overflow-x:auto"><table id="ixladopen"></table></div></div>
...
<div class="pan">
  ...
  <div id="fxladopenwrap" style="display:none;margin-top:6px"><div class="lbl" style="color:var(--grn)">OPEN NOW (live legs)</div><div style="overflow-x:auto"><table id="fxladopen"></table></div></div>
  <div id="fxladtradeswrap" style="display:none;margin-top:6px"><div class="lbl">TRADES LOG (completed forward clips)</div><div style="overflow-x:auto"><table id="fxladtrades"></table></div></div>
</div>
</div><!-- /ladder cstack -->
...
</div><!-- /compcols (whole-desk two-column layout, S-2026-07-22h) -->
...
/* S-2026-07-20o (operator: "not hearing anything"): Chrome autoplay-blocks WebAudio until a
   user gesture OR the site is allowlisted (lock icon -> Site settings -> Sound -> Allow). The
   old chime() dropped blocked bells SILENTLY (resume() stays pending, catch never fires) --
   the operator had no way to tell "no events" from "events fired but muted by the browser".
   Now a dropped bell turns the SND button red 'SND BLOCKED' + one-shot toast with the fix;
   the first successfully-played bell restores the button. */
var __sndBlockedShown=false;
function __sndBlocked(){var b=el('snd');if(b){b.textContent='SND BLOCKED';b.style.color='var(--redB)';}
 if(__sndBlockedShown)return;__sndBlockedShown=true;
 __almToast('bell DROPPED — Chrome is blocking sound. Click the page once, or lock icon → Site settings → Sound → Allow, then reload.','fail');}
function chime(notes,_r){if(!SND)return;ensureCtx();if(!ACTX)return;
 if(ACTX.state==='suspended'&&!_r){ACTX.resume().then(function(){chime(notes,1);}).catch(function(){});
  setTimeout(function(){if(ACTX&&ACTX.state==='suspended')__sndBlocked();},600);return;}
 if(__sndBlockedShown){__sndBlockedShown=false;var _b=el('snd');if(_b)_b.style.color='';sndBtn();}
...
/* S-2026-07-20o: restart was toast-only (bellFn=null) -- operator wants it AUDIBLE. Distinct
   low-high-low-high pattern so it can't be confused with entry/win/loss. */
function restartBell(){chime([[0,523,0.5],[0.15,392,0.45],[0.3,523,0.5],[0.45,784,0.5]]);}
...
function pollFires(){var now=Date.now();
 ...
 var cc=window._cc||{};var loadArmed={};
 for(var ct in cc){var p=cc[ct];if(!p)continue;
  /* S-18aj FIX: key on ARMED, not subleg existence — idle books ALWAYS emit their subleg
     rows, so every key baselined at page load and an idle->ARMED transition created no new
...
(function(){var z=[[0,5,'#1d2733'],[5,7,'#143042'],[7,13,'#155446'],[13,13.5,'#0F6E56'],[13.5,15,'#7a5a14'],[15,20,'#155446'],[20,22,'#143042'],[22,24,'#1d2733']];
 ...
 /* S-2026-07-22k operator: the time cursor was a dim 2px sliver — NEON bar now:
    wider, glowing green, unmissable against every session segment. */
 el('sessbar').innerHTML=h+'<div id="sessmk" style="position:absolute;top:-2px;bottom:-2px;width:4px;border-radius:2px;background:#39ff14;box-shadow:0 0 4px #39ff14,0 0 10px #39ff14,0 0 18px rgba(57,255,20,0.7)"></div>';tickClk();})();
...
(function(){var h='';TKS.forEach(function(t){
 ...
 h+='<div id="tkbox_'+t[0]+'" style="padding:6px 12px;border-right:1px solid var(--bd);min-width:104px">'
  +'<div class="lbl" id="tkl_'+t[0]+'">'+t[1]+'</div><div class="num" id="tk_'+t[0]+'" style="font-size:13px;color:'+(_sl>0?'var(--t2)':'var(--w)')+'">'+_si+'</div>'
 ...
 h2+='<div id="ctkbox_'+t[0]+'" style="padding:6px 12px;border-right:1px solid var(--bd);min-width:104px">'
  +'<div class="lbl" id="ctkl_'+t[0]+'">'+t[0]+'</div><div class="num" id="ctk_'+t[0]+'" style="font-size:13px;color:var(--w)">—</div>'
...
function cxApply(sym,px,chg,hi,lo){var t=CTKS.find(function(x){return x[1]===sym;});if(!t)return;
 if(px>0){el('ctk_'+t[0]).textContent=px.toLocaleString(undefined,{maximumFractionDigits:px>100?1:px>1?3:5});
  (window._cpx=window._cpx||{})[t[0]]=px;/* last live px per coin — mirror-PnL calc in drawProx */}
 /* S-2026-07-18ah trigger-PROXIMITY owns the sub-line + bar when the coin has live
    mimic-cell detector data (operator: "those 24h % mean nothing — show me when it is
    getting close to trading"). 24h% + day-range only remain as fallback for coins with
    NO self-detect companion cell (XSEC-only tiles) — never fight drawProx's writes. */
 if(window._proxHas&&window._proxHas[t[0]])return;
...
/* ── S-2026-07-18ah trigger-PROXIMITY on the crypto symbol boxes ──────────────────────
   Per coin: the BEST (closest-to-trigger) live self-detect mimic cell — max win_move/thr
   ratio over that coin's prox_valid books (josgp1 emit_companion_state, 5s throttle ->
   120s relay -> /api/crypto_companion, fields win_move_pct/det_thr_pct/win_open/
   confirm_dist_bp). A window-OPEN cell outranks all ratios (already triggered — show it).
   Tile sub-line: "→0.32/0.50% 64%" + bar fill = ratio, ramp grey->amber(60%)->red(85%);
   window open = green "WIN +j%" (+ "conf −Xbp" while the flat book still needs confirm).
   Read-only view of pre-trade-book detector state — NOT PnL, no-backtest-in-live-GUI clean. */
window._proxHas={};
/* real-mirror unrealized $ for a coin: live_mirror holds (real Binance fills, S-18al) marked at
   the live WS price. null = no real hold OR no price yet (render nothing, not $0.00). */
function mirPnl(coin){var mir=window._ccMirror;if(!mir||!Array.isArray(mir.holds))return null;
 var px=(window._cpx||{})[coin];if(!(px>0))return null;
 var tot=0,got=false;
 mir.holds.forEach(function(h){if(h&&h.coin===coin&&safe(h.qty)>0&&safe(h.px)>0){tot+=h.qty*(px-h.px);got=true;}});
 return got?tot:null;}
function ccProx(){var live=window._cc||{},best={};
 Object.keys(live).forEach(function(tg){var p=live[tg];
  if(!p||!p.sym||p.retired)return;
  /* S-18aj: TRADING outranks everything — an armed leg is an OPEN BE-floored book; the tile
     must show the BOOK state, not a proximity ratio or the 24h% (COMP fired 5 legs while its
     tile sat on a red 24h number). */
  if(p.armed){var b0=best[p.sym];
   if(b0&&b0.trading){b0.n++;b0.pk=Math.max(b0.pk,safe(p.peak_mfe_pct));}
   else best[p.sym]={trading:true,n:1,pk:safe(p.peak_mfe_pct)};
   return;}
  if(!p.prox_valid)return;
  var thr=safe(p.det_thr_pct);if(!(thr>0))return;
  var j=safe(p.win_move_pct),b=best[p.sym];
  if(b&&b.trading)return;
  if(p.win_open){
   if(!b||!b.open||j>b.j)best[p.sym]={open:true,j:j,thr:thr,conf:safe(p.confirm_dist_bp)};
  }else if(!b||(!b.open&&j/thr>b.j/b.thr)){
   best[p.sym]={open:false,j:j,thr:thr,conf:0};
  }});
 return best;}
function drawProx(){var best=ccProx();var has={};
 CTKS.forEach(function(t){var b=best[t[0]],pe=el('ctkp_'+t[0]),r=el('ctkr_'+t[0]),bx=el('ctkbox_'+t[0]);
  if(!pe||!r)return;
  /* S-18aj: tile box lights per state (operator: "why is it not lit up better") — reset first */
  if(bx){bx.style.outline='';bx.style.outlineOffset='';bx.style.background='';bx.style.boxShadow='';bx.style.animation='';}
  var mp=mirPnl(t[0]);
  var holding=(mp!==null);/* real open Binance leg on this coin (live_mirror) = LIVE TRADING */
  /* S-2026-07-19t (operator: "anything trading highlighted, ALL symbols"): a real live_mirror
     hold is the AUTHORITATIVE trading signal -> the tile goes neon independently of the _cc
     detection relay (which can lag / carry no armed flag). Fixes SUI/SUSHI/TRX trading live but
     dark because _cc had no armed cell for them. */
  var trading=(b&&b.trading)||holding;
  if(!b&&!holding)return;/* no self-detect cell AND no real hold -> 24h% fallback keeps the slot */
  has[t[0]]=1;
  if(trading){
   /* S-18ak: TRUE NEON. Same #39ff14 + neonPulse as the header beacon. */
   if(bx){bx.style.outline='2px solid #39ff14';bx.style.outlineOffset='-2px';
    bx.style.background='rgba(57,255,20,.14)';
    bx.style.boxShadow='inset 0 0 14px rgba(57,255,20,.5), 0 0 12px #39ff14, 0 0 26px rgba(57,255,20,.55)';
    bx.style.animation='neonPulse 1.2s ease-in-out infinite';}
...
R"OMEGAD4(  var dollar='<span style="font-weight:700;color:'+(mp>=0?'#39ff14':'var(--redB)')+'">$'+(mp>=0?'+':'&minus;')+fmt2(Math.abs(mp),2)+'</span>';
  ...
  /* real hold, no self-detect open/trading state (legacy pre-narrowing mark-to-market) */
  pe.innerHTML=dollar;
  r.style.width='100%';r.style.background=mp>=0?'#39ff14':'var(--redB)';});
 window._proxHas=has;}
setInterval(drawProx,1000);
...
/* S-2026-07-20: + IBKR micro-contract keys. Live ca_engines (NqMomoIbkr, MgcVolBreakoutM30)
   emit their OWN symbol ("MNQ"/"MGC") into open_positions/REGPOS, NOT the display sym -- so a
   live NQ/gold futures trade never lit its tile. MNQ/NQ->nq, MGC/NQ->gold, MES->sp, MYM->dj. */
var TK_SYM2KEY={XAUUSD:'gold','XAUUSD.M':'gold',MGC:'gold',US500:'sp','US500.F':'sp',MES:'sp',USTEC:'nq','USTEC.F':'nq',NAS100:'nq',MNQ:'nq',NQ:'nq','DJ30':'dj','DJ30.F':'dj',MYM:'dj',GER40:'ger30',UK100:'uk100',USOIL:'cl','USOIL.F':'cl',XAGUSD:'xag',EURUSD:'eurusd',GBPUSD:'gbpusd',USDJPY:'usdjpy',AUDUSD:'audusd',NZDUSD:'nzdusd',USDCAD:'usdcad'};
function markTiles(){
 var act={};
 ...
 /* S-2026-07-19t: a real live_mirror hold IS trading -> label neon too, independent of _cc
    (operator: ALL trading symbols highlighted). */
 var _mh=window._ccMirror;if(_mh&&Array.isArray(_mh.holds))_mh.holds.forEach(function(h){
  if(h&&safe(h.qty)>0){var c=String(h.coin||'').toUpperCase();if(c)ca[c]=1;}});
 CTKS.forEach(function(t){var e=el('ctkl_'+t[0]);if(e){e.style.color=ca[t[0]]?'#39ff14':'';
  e.style.textShadow=ca[t[0]]?'0 0 8px #39ff14':'';e.style.fontWeight=ca[t[0]]?'700':'';}});
}
...
function render(J){lastJ=J;
 ...
 (function(){var boot=Math.round((Date.now()/1000-up)/60)*60,
  ...
  if(Math.abs(boot-seen)>120){localStorage.setItem('omega_boot_seen',String(boot));
   almRing('Omega service restarted (boot epoch moved) — sound re-armed','info',restartBell);/* S-20o: audible (was toast-only) */
 ...
 (function(){var g=el('goldreg');if(!g)return;
  var gr=(J.gold_regime||'?'),blk=safe(J.gold_long_blocked),wm=(J.gold_warm===undefined?1:safe(J.gold_warm));
  // wm=0 => genuinely cold-warming (the ONLY fault state). Otherwise engines are seeded+ARMED;
  // blk=1 => held by regime (normal protection), blk=0 => longs live.
  if(!wm){g.textContent=gr+' · WARMING';g.style.color='var(--redB)';g.title='FAULT: gold regime brain not warm — engines cold. Should never happen post-seed; investigate seeding.';}
  else if(blk){g.textContent=gr+' · long ARMED·held';g.style.color='#e0a020';}
  else{g.textContent=gr+' · long ARMED';g.style.color='var(--grnB)';}
 })();
 ...
 TKS.forEach(function(t){var b=safe(J[t[0]+'_bid']),a=safe(J[t[0]+'_ask']);
  ...
  /* FX majors need 5dp (EUR/GBP/AUD/NZD trade sub-2.0 so the default b>100?1:4 gives
     1-pip resolution -- masks sub-pip moves + collapses the spread to "s 0.00000").
     USDJPY ~162 would hit the b>100?1 branch (1dp) so pin it to 3dp. S-2026-07-06. */
  var _FXP={eurusd:5,gbpusd:5,audusd:5,nzdusd:5,usdjpy:3};
  var _dig=(_FXP[t[0]]!==undefined)?_FXP[t[0]]:(b>100?1:4);
  ...
  if(b>0){try{localStorage.setItem('tklast_'+t[0],String(b));}catch(_){}
   tke.textContent=b.toLocaleString(undefined,{maximumFractionDigits:_dig});tke.style.color='var(--w)';}
  else{var lv=parseFloat((function(){try{return localStorage.getItem('tklast_'+t[0]);}catch(_){return '';}})());
   if(!(lv>0)&&SEED_LAST[t[0]])lv=SEED_LAST[t[0]];
   if(lv>0){tke.textContent=lv.toLocaleString(undefined,{maximumFractionDigits:(_FXP[t[0]]!==undefined)?_FXP[t[0]]:(lv>100?1:4)});tke.style.color='var(--t2)';}
 ...
 /* DOM — XAUUSD L2 populator removed S-2026-07-22 (panel deleted above; gold L2 depth feed
    retired S-2026-07-10). gold_bids/gold_asks/l2_gold no longer consumed by the desk. */

 /* MICROSTRUCTURE + BROKER + SIGNAL TAPE populators removed 2026-07-04 (panels deleted, operator: irrelevant). */

 var lts=J.live_trades||[];window._lastLts=lts;/* tile trigger-marker reads this */
 /* neon TRADING beacon: open live trade(s) => bright #39ff14 pulsing glow (unmissable);
    flat => dim IDLE. Driven by live_trades only -- pre-trade/paper books never light it. */
 (function(){var d=el('trdot'),l=el('trdlbl');if(!d||!l)return;
  if(lts.length){d.style.background='#39ff14';d.style.boxShadow='0 0 4px #39ff14,0 0 10px #39ff14,0 0 20px #39ff14';d.style.animation='neonPulse 1.2s ease-in-out infinite';
   l.textContent='TRADING ×'+lts.length;l.style.color='#39ff14';l.style.textShadow='0 0 8px #39ff14';}
  else{d.style.background='var(--t3)';d.style.boxShadow='none';d.style.animation='none';
   l.textContent='IDLE';l.style.color='';l.style.textShadow='';}})();
...
function pollTrades(){
  Promise.all([
    ...
    /* S-2026-07-15n operator: StockDip/StockTurtle book trades were in the TOTAL (sdAll) but never in
       the trade LIST. Flatten books[].trades[] into trade rows (each carries entry_ts/exit_ts) so
       MU/DELL etc. show WITH their timestamps and count in the total. */
    ...
    var live=(((res[3]||{}).trades)||[]).map(function(t){
      return {book:'chimera',engine:'MIMIC-LIVE',symbol:t.sym||'',side:'long',
    ...
    /* S-2026-07-20 (operator: "why do these nas trades not appear in last 5 trade list"): the
       INDEX/FX mimic LADDER forward clips (idxladder/fxladder_companion pairs[].trades[]) were
       fetched only by pollIxLad/pollFxLad for their own panels -- never folded into "all books".
       They are REAL forward $ clips (like the stock book / EDGE), so list them. Money still totals
       via the separate _ixladtot/_fxladtot (ladTot, realized-only) -> rows here are display+count
       only, no double-count into ALL-TIME. */
    var lad=[];[window._ixlad,window._fxlad].forEach(function(j){((j&&j.pairs)||[]).forEach(function(p){
      (p.trades||[]).forEach(function(t){lad.push({book:'ladder',engine:(t.flavor||((p.pair||'')+'Lad')),
        symbol:p.pair||p.live_sym||'',side:t.dir||'long',price:safe(t.entry),exitPrice:safe(t.exit),
        net_pnl:safe(t.usd),exitReason:t.reason||'',entryTs:safe(t.entry_ts),exitTs:safe(t.exit_ts)});});});});
    var all=(res[0]||[]).concat(cryptoDisp).concat(sd).concat(live).concat(lad);
    /* S-2026-07-12 operator: fold the CHIMERA EDGE realized (e.g. TIA-TSMOM +7.65) into the
       crypto PnL — it was showing in LAST-15 but never in the total. EDGE trades only
       ...
       stamp SUB-LEG tags (cfg tag + -T/-S/-L/-J<n>, MimicLadderCompanion emit_clip_) with no
       ...
       (TP/SL/TIMEOUT/TRAIL/CORE_EXIT/REBAL...) match neither, so EDGE rows still count. */
    window._chimtot=(res[1]||[]).reduce(function(s,t){var e=(t.engine||''),xr=(t.exitReason||'');return (/CLIP|MIMIC-LIVE/i.test(e)||/-[TSLJ]\d+$/.test(e)||/CLIP|CUT|ENGINE_EXIT/i.test(xr))?s:s+safe(t.net_pnl!=null?t.net_pnl:t.pnl);},0);/* MIMIC-LIVE rows excluded: their cash realized folds via _clivetot (S-2026-07-18r), rows here are display-only */
    /* S-19c (operator "COMP-COMP...36.35" recurrence #3): companion CLIP rows are banked in _cctot
       as PAPER bp (S-19a/19b) -- isPaperClip() flags them so they're dropped from LAST-15 entirely
       (see cryptoDisp above), not shown alongside genuine $ rows (OMEGA/stock/EDGE/MIMIC-LIVE).
       MIMIC-LIVE is excluded here -- that's real Binance cash (_clivetot), not PAPER. */
    function isPaperClip(t){var e=(t.engine||''),xr=(t.exitReason||'');
      if(/MIMIC-LIVE/i.test(e))return false;
      return /CLIP/i.test(e)||/-[TSLJ]\d+$/.test(e)||/CLIP|CUT|ENGINE_EXIT/i.test(xr);}
    if(typeof updDayPnl==='function')updDayPnl();
    all.forEach(function(t){ if(!t.exitTs && t.exit_ts_utc){ var d=Date.parse((t.exit_ts_utc||'').replace(' UTC','Z').replace(' ','T')); if(!isNaN(d))t.exitTs=d/1000; } });
    all.sort(function(a,b){return (b.exitTs||0)-(a.exitTs||0);});
    /* S-2026-07-15n operator ("include the mimic trades in that total + show how many mimic trades
       we had, i.e. 2 or 4 or 1"): count ALL closed trades across every book, and how many are MIMIC
       engines (*Mimic tag). Shown next to the panel title. */
    var nAll=all.length, nMimic=all.filter(function(t){return /mimic/i.test(t.engine||'');}).length,
        nStock=sd.length, nCrypto=cryptoDisp.length, nLad=lad.length;
    var cn=document.getElementById('alltradesn');
    if(cn)cn.innerHTML='· '+nAll+' total ('+nMimic+' mimic'+(nStock?' · '+nStock+' stock':'')+(nCrypto?' · '+nCrypto+' crypto':'')+(nLad?' · '+nLad+' ladder':'')+')';
    ...
    var rows=top.map(function(t){
      ...
      var isMim=/mimic/i.test(t.engine||'');
      /* S-2026-07-15s (operator "shown in the correct box"): name the ACTUAL stock sub-book, not a
      ...
      var tag=t.book==='crypto'?'<span style="color:var(--w)">crypto</span> ':(t.book==='chimera'?'<span style="color:#c084fc">chimera</span> ':(t.book==='stock'?'<span style="color:#5cc8ff">'+stkLbl+'</span> ':'')); /* chimera = josgp1 PRE-TRADE closed trades; stock = StockDip/Turtle book */
      ...
      var bk=ind+tag+esc(t.engine||'')+(isMim?' <span class="d" style="font-size:9px">mimic</span>':'');
...
/* ── crypto mimic books — DYNAMIC render, no baked roster (S-2026-07-13 panel rewrite) ──
   ...
   *-SWEET (validated 2023+ sweet-spot self-detect cells, HANDOFF 2026-07-13i). The old
   ...
   the panel CANNOT drift from the box again. Edge-engine (TSMOM/ICHI/KELT slot) closes
   are NOT mimic books; they arrive as chimera rows in LAST-15.
...
function ccWbp(s){var w=(s&&s.bank_bp_real_w!==undefined)?safe(s.bank_bp_real_w):safe(s&&s.bank_bp_real)*((s&&s.mult!==undefined)?safe(s.mult):1);return Math.max(0,w);}
/* Row model (dynamic): group served legs by coin — coin summary line, then one nested
   `↳` row per BOOK (tag) with its type/state/peak/clips/bank, then open cascade sublegs
   nested one level deeper. Banks Σ over ALL served legs -> _cctot fold (unchanged shape). */
function ccState(s){
 if(s.retired)return '<span style="color:var(--red);font-weight:600">RETIRED</span>';
 var n=(s.sublegs&&s.sublegs.length)||0;
 if(n)return '<span class="g" style="font-weight:600">OPEN ('+n+' leg'+(n>1?'s':'')+')</span>';
 if(s.armed)return '<span class="g">ARMED</span>';
 /* S-2026-07-18ah window-state (carried ask #1 + trigger-proximity): a det window OPEN
    with the book still flat means "jump seen, waiting for confirm-cross" — show it +
    the bp still needed; an idle self-detect book shows its live j/thr proximity. */
 if(s.win_open)return '<span style="color:var(--amb);font-weight:600">WINDOW OPEN</span>'
  +(safe(s.confirm_dist_bp)>0?'<span class="d"> · +'+fmt2(safe(s.confirm_dist_bp),0)+'bp to entry</span>':'');
 if(s.prox_valid&&safe(s.det_thr_pct)>0){
  var _pj=safe(s.win_move_pct),_pr=Math.max(0,Math.min(1,_pj/safe(s.det_thr_pct)));
  return '<span class="d">idle · &rarr;'+fmt2(_pj,2)+'/'+fmt2(safe(s.det_thr_pct),1)+'% ('+Math.round(_pr*100)+'%)</span>';
 }
 return '<span class="d">idle · awaiting jump</span>';
}
function ccKind(s){
 var tag=String(s.tag||''),cell=String(s.cell||'');
 if(cell==='REGIME_SWITCH')
  return 'BE-mimic of '+(s.parent_w!==undefined?s.parent_w:24)+'h/+'+fmt2(s.parent_thr_pct!==undefined?s.parent_thr_pct:8,0)+'% regime parent';
 if(tag.indexOf('SWEET')>=0||cell.indexOf('SWEET')>=0)
  return 'sweet-spot self-detect '+(s.det_w!==undefined?s.det_w:'?')+'h/+'+fmt2(s.det_thr_pct,1)+'%';
 return cell||'mimic';
}
function drawCC(){
 /* Panel + DOM rendering REMOVED S-2026-07-19g (see markup-side comment above cstack).
    _cctot is still computed and stored -- harmless, already excluded from ALL-TIME by S-18w --
...
/* ── CC truth chip (S-2026-07-18af): chimera executor VERIFIED state on the header.
   Feed: Mac chimera_executor_watch.sh (1min probe of josgp1: build==HEAD, mode LIVE,
   restart-loop, config-conflict) -> relay HOP 4 -> /api/chimera_health. Fail-closed:
   missing feed, stale ts (>6min: watch 1min + relay 120s + margin) or ok=0 all render
   RED — a dead watch/relay must NEVER read as green (the 07-18 93-restart loop hid
   behind exactly that). Health chip, not PnL — no-backtest-in-live-GUI rule untouched. */
function pollCH(){fetch('/api/chimera_health').then(function(r){return r.json();}).then(function(j){
 var d=el('ccdot'),l=el('cclbl');if(!d||!l)return;
 function red(t,tip){d.style.background='var(--red)';d.style.boxShadow='0 0 4px var(--red),0 0 10px var(--red)';
  d.style.animation='neonPulse 1.2s ease-in-out infinite';l.textContent=t;l.style.color='var(--red)';
  l.style.textShadow='0 0 8px var(--red)';if(tip)l.title=tip;}
 if(!j||!safe(j.ts)){red('CC NO-DATA','chimera health feed absent — watch/relay never delivered');return;}
 var age=Date.now()/1000-safe(j.ts);
 if(age>360){red('CC STALE '+Math.round(age/60)+'m','chimera health feed stale — Mac watch or relay dead; executor state UNKNOWN');return;}
 if(!safe(j.ok)){red('CC RED: '+String(j.reason||'?').slice(0,70),String(j.reason||''));return;}
 var us=safe(j.uptime_s),uh=Math.floor(us/3600),um=Math.floor(us%3600/60);
 d.style.background='#39ff14';d.style.boxShadow='0 0 4px #39ff14,0 0 8px #39ff14';d.style.animation='none';
 l.textContent='CC '+(j.build||'?')+' '+(j.mode||'?')+' up='+(uh>0?uh+'h'+um+'m':um+'m')+' ✓';
 l.style.color='#39ff14';l.style.textShadow='0 0 6px #39ff14';l.title='chimera VERIFIED: build '+(j.build||'?')+' == josgp1 HEAD '+(j.headsha||'?')+', mode '+(j.mode||'?')+', starts30='+safe(j.starts30)+', probe age '+Math.round(age)+'s';
}).catch(function(){var l=el('cclbl');if(l){l.textContent='CC ?';l.style.color='var(--amb)';}});}
setInterval(pollCH,15000);pollCH();
...
/* ── STOCK MOVERS (per-name BIGCAP mimic LADDER · long-only · no-floor · native C++ · additive, STANDALONE) ──
   ...
   validated no-floor giveback ladder (bigcap_mimic_ladder_bt.py: +7,044% of clip notional PF1.58 all-6).
   ...
   Schema: {engine,pre-trade,grade,total_usd_real,ts,names:[{sym,bars,notional,clips,wins,pct_real,usd_real,
...
function drawStockMover(){var j=window._sm||null;
 ...
 if(!names.length){el('smtab').innerHTML=h+'<tr><td class="l d" colspan="6">wired · waiting for first state write (39 names, deploy-forward · $0 until first forward clip closes)</td></tr>';
  el('sminfo').textContent='native C++ · pre-trade · daily-close · real forward trades only';renderCompanionOpenTrades('sm',[],[],2);return;}
...
R"OMEGAD7(  (p.open||[]).forEach(function(o){desk+=safe(o.upnl_usd_real!==undefined?o.upnl_usd_real:o.upnl_usd);allOpen.push(o);});
 ...
 el('sminfo').innerHTML='DESK <span style="color:'+dcol+';font-weight:600">'+fmt$(desk)+'</span> forward · '
   +allTrades.length+' closed clip'+(allTrades.length===1?'':'s')+openTxt+' · '+names.length+' names ('+nact+' active) · daily-close · pre-trade · ladder no-floor';
...
function pollStockMover(){fetch('/api/stockladder_companion').then(function(r){return r.json();}).then(function(j){
 ...
 window._smtot=(((window._sm||{}).names)||[]).reduce(function(s,p){return s+safe(p.usd_real);},0);/* S-2026-07-10: ALL-TIME folds REALIZED forward clips ONLY (usd_real). Open-leg unrealized MTM removed from the header -- pre-trade bigcap-mover ladder legs marking red dragged ALL-TIME as a phantom drop; per-panel DESK still shows uPnL. */
 drawStockMover();
...
/* ── STOCK DIP/TURTLE single-name daily-close LONG book (native C++ · pre-trade · additive · STANDALONE) ──
   ...
   own /api/stockdipturtle endpoint + _sdtot global + sdAll fold into totT + STOCK class. REALIZED forward
   clips ONLY (usd_real), same rule as _smtot. Judge STANDALONE ([[feedback-companion-independent-engine]]). */
function pollStockDip(){fetch('/api/stockdipturtle').then(function(r){return r.json();}).then(function(j){
 if(j&&(j.books||[]).length)window._sd=j;
 window._sdtot=(((window._sd||{}).books)||[]).reduce(function(s,p){return s+safe(p.usd_real);},0);/* ALL-TIME folds REALIZED clips only (usd_real) */
 if(typeof updDayPnl==='function')updDayPnl();if(typeof drawLedger==='function')drawLedger();
 }).catch(function(){});}
setInterval(pollStockDip,15000);pollStockDip();

/* ── BIGCAP HI52 portfolio paper book (S-2026-07-17m). Schema: {engine,pre-trade,pool_usd,pnl_usd,
...
function drawHi52(){var j=window._hi52||{},t=el('hi52tab');if(!t)return;
 ...
 var h='<tr><td class="l lbl">member (within 5% of 52wk high)</td><td class="lbl">weight</td></tr>';
 if(!mem.length){t.innerHTML=h+'<tr><td class="l d" colspan="2">'+(on?'no members within 5% of 52wk high — flat':'SPY &lt; 200DMA — portfolio flat by gate')+'</td></tr>';return;}
 t.innerHTML=h+mem.map(function(m){return '<tr><td class="l">'+esc(m.sym||'')+'</td><td class="num">'+(100*safe(m.w)).toFixed(1)+'%</td></tr>';}).join('');}
function pollHi52(){fetch('/api/hi52').then(function(r){return r.json();}).then(function(j){
 if(j&&j.engine)window._hi52=j;
 window._hi52tot=safe((window._hi52||{}).pnl_usd);/* portfolio paper P&L -> ALL-TIME (rdagent-class fold; deploy-forward so 0 at wire) */
 drawHi52();
 if(typeof updDayPnl==='function')updDayPnl();if(typeof drawLedger==='function')drawLedger();
 }).catch(function(){drawHi52();});}
setInterval(pollHi52,30000);pollHi52();

/* ── FX + INDEX mimic LADDER panels (S-2026-07-07x wire; /api/fxladder_companion +
   /api/idxladder_companion, written in-binary by FxMimicLadderCompanion registries).
   FORWARD TRADES ONLY — deploy-forward, $0 until a live clip closes; per-clip rt cost
   debited in-engine (single honest column). STANDALONE additive, never vs-WIDE.
   Schema: {engine,pre-trade,grade,total_usd,ts,pairs:[{pair,live_sym,bars,deploy_ts,ts,W,thr,
   rt_cost_bp,notional,win:{active,age,nclips},clips,wins,pct,usd,open:[..],trades:[..],tiers:[..]}]}. */
function drawLadder(pfx,j,label){
 var h='<tr><td class="l lbl">pair</td><td class="lbl">win</td><td class="lbl">legs</td>'
      +'<td class="lbl">clips</td><td class="lbl">wins</td><td class="lbl">book $</td></tr>';
 var pairs=(j&&j.pairs)||[];
 if(!pairs.length){el(pfx+'tab').innerHTML=h+'<tr><td class="l d" colspan="6">wired · waiting for first state write (deploy-forward · $0 until first forward clip closes)</td></tr>';
  el(pfx+'info').textContent='native C++ · pre-trade · H1 intrabar · forward trades only';renderCompanionOpenTrades(pfx,[],[],5);return;}
 ...
 el(pfx+'info').innerHTML='DESK <span style="color:'+dcol+';font-weight:600">'+fmt$(desk)+'</span> forward · '
   +allTrades.length+' closed clip'+(allTrades.length===1?'':'s')+openTxt+' · '+pairs.length+' '+label+' ('+nact+' active) · H1 · pre-trade · BE-floored-on-open';
...
function pollFxLad(){fetch('/api/fxladder_companion').then(function(r){return r.json();}).then(function(j){
 ...
 drawLadder('fxlad',window._fxlad,'pairs');
 if(typeof updDayPnl==='function')updDayPnl();if(typeof drawLedger==='function')drawLedger();
 }).catch(function(){drawLadder('fxlad',window._fxlad,'pairs');});}
setInterval(pollFxLad,15000);pollFxLad();
function pollIxLad(){fetch('/api/idxladder_companion').then(function(r){return r.json();}).then(function(j){
 if(j&&(j.pairs||[]).length)window._ixlad=j;
 window._ixladtot=ladTot(window._ixlad);
 drawLadder('ixlad',window._ixlad,'indices');
 if(typeof updDayPnl==='function')updDayPnl();if(typeof drawLedger==='function')drawLedger();
...
R"OMEGAD8( /* FULL PnL (operator 2026-07-05d: "the Omega pnl should have the crypto profit added, that is
 ...
 /* S-2026-07-18w operator ("WHY DO WE HAVE A PHANTOM +49"): crypto PAPER (pre-trade) $ is OUT of
    the money headline. ._cctot (mimic bank bp, paper) + ._chimtot (chimera PRE-TRADE closes, paper)
 ...
 /* 3. OMEGA stall-clip (cAll) — S-2026-07-22 operator mandate (real-fills-only): the stall-clip
       companion bank is a SHADOW/companion COMPUTED book (ENGINE HEAT labels it "pre-trade paper
       PnL"), NOT real broker fills. It stays shown in the ENGINE HEAT + OMEGA book panels below
       (which read window._gcPer directly) but is OUT of the live-money class chips, exactly like
       the TODAY/ALL-TIME headline. So cls.gold/cls.stock get NOTHING here — only cls.crypto (real
       Binance fills, folded above) is nonzero. This zeroes the STOCK/GOLD chips and drops the
       ±unclassified remainder to 0 (cls4 == totT == clvAll). When Omega books REAL fills, fold
       that real bank here per-class — never the stall-clip paper. */
 /* 4. reconciliation — remainder is DEFINED as totT minus the 4 class sums, so it captures any
       unclassified ledger row ('other') or stall-clip reconciliation gap. Surface it, never hide it. */
 var cls4=cls.stock+cls.crypto+cls.fx+cls.gold,rem=totT-cls4;
 tweenNum('pnl_stk',cls.stock,fmt$);el('pnl_stk').style.color=cls.stock>=0?'var(--grn)':'var(--red)';
 tweenNum('pnl_cry',cls.crypto,fmt$);el('pnl_cry').style.color=cls.crypto>=0?'var(--grn)':'var(--red)';
...
function drawHeat(){var rs=winRows();var by={};
 ...
 order.forEach(function(g){if(!groups[g])return;
  ...
  h+='<div class="lbl" style="margin:6px 0 3px">'+g+(g==='FX'?' — BE-floor companion (pre-trade)':'')+' <span style="color:var(--t3)">'+groups[g].length+' active</span></div>'
...
function drawPromo(){if(!el('promo'))return;/* PROMOTION TRACKER panel removed 2026-07-06 (operator: useless) */var by={};
 ...
 if(!ks.length){el('promo').innerHTML='<span class="d">no pre-trade history yet</span>';return;}
 var GATE=30,h='';
...
R"OMEGAD10(  ctx.beginPath();ctx.moveTo(x,Y(b[2]));ctx.lineTo(x,Y(b[3]));ctx.stroke();ctx.globalAlpha=1;
 ...
 /* ── closed-trade overlay v2: glowing entry triangles, ringed exits,
    ...
    newest exit. Trades come from the pre-trade ledger (ROWS). ── */
 (function(){
 ...
 var stale=barAge>=0&&barAge>2*tfsec+90;
