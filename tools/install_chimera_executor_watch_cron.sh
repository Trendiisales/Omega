...
# for tools/chimera_executor_watch.sh (every 10 min). Pattern per
# feedback-crontab-edit-via-script: backup first, marker-managed line, rerun-safe.
# Remove: `$0 remove`.
set -euo pipefail
MARKER="# OMEGA-CHIMERA-EXECUTOR-WATCH"
LINE="*/10 * * * * /bin/bash /Users/jo/Omega/tools/chimera_executor_watch.sh >> /tmp/chimera_executor_watch_cron.log 2>&1 $MARKER"
