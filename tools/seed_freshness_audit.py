...
print(f"active seed CSVs found: {len(seed_paths)}  ok: {len(ok)}  stale: {len(stale)}  missing/unreadable: {len(missing)}\n")
...
if stale:
    print(f"\n*** {len(stale)} STALE SEED(S) -- an engine/gate is operating on a price view detached from reality. FIX. ***")
    sys.exit(1)
print("\nAll active seeds fresh.")
sys.exit(0)
