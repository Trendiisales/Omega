...
# ── execution-outcome probe (the no-trade-despite-signals blind spot) ──────────
# A per-ENGINE loss miner is structurally blind when a system produces 0 CLOSED TRADES:
# there is simply no row to group, so the engine that fired signals but got 0 fills
# (overnight 2026-07-23: every crypto order bounced off a Binance -1013 LOT_SIZE filter)
# generates NO section and vanishes. Before mining, probe the live executor for the
# "intents>0 but fills==0 / reject storm" condition and surface it as a report NOTICE —
# reusing the exact check trade_sentinel.py acts on (single source of truth, no drift).
def exec_outcome_probe(system: str) -> tuple[list[dict], dict | None]:
    if system not in ("chimera", "both"):
        return [], None
    try:
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        import trade_sentinel as _ts  # same package dir
        lines = _ts.pull_exec_events()
        return _ts.check_execution_outcome(lines)
    except Exception as e:  # noqa: BLE001
        print(f"  WARN: exec-outcome probe failed: {e}", file=sys.stderr)
        return [], None


# ── report ──────────────────────────────────────────────────────────────────────
def render_report(all_results: list[dict], out_path: Path, notices: list[str] | None = None) -> None:
    lines = []
...
def main():
    ...
    if args.csv:
        ...
        if args.system in ("omega", "both"):
            ...
            f = load_omega(); sys_rows["omega"] = len(f); frames.append(f)
        if args.system in ("chimera", "both"):
            print("Pulling Chimera ledger (read-only ssh)...", file=sys.stderr)
            f = load_chimera(); sys_rows["chimera"] = len(f); frames.append(f)

    # OUTCOME probe FIRST — independent of whether any trade closed. Catches the case a
    # trade-based miner cannot see: signals/intents fired but 0 fills (reject storm).
    outcome_alerts, exec_counts = ([], None) if args.csv else exec_outcome_probe(args.system)
    notices: list[str] = []
    for s, n in sorted(sys_rows.items()):
        if n == 0:
            notices.append(f"> ⚠️ **ZERO-ACTIVITY — `{s}` returned 0 closed trades.** A per-engine "
                           "loss miner has nothing to group here, so any engine that fired signals "
                           "but did not fill is INVISIBLE to the mining below. See the execution "
                           "outcome probe (this is the overnight no-fill blind spot).")
    if exec_counts is not None:
        if outcome_alerts:
            notices.append(f"> 🚨 **EXECUTION OUTAGE ({outcome_alerts[0]['code']}).** Executor window: "
                           f"{exec_counts['intents']} intents · {exec_counts['fills']} fills · "
                           f"{exec_counts['reject_orders']} rejected · {exec_counts['lot1013']}× "
                           f"LOT_SIZE/-1013. {outcome_alerts[0]['msg']}")
        else:
            notices.append(f"> ✅ Execution outcome OK: {exec_counts['intents']} intents → "
                           f"{exec_counts['fills']} fills, {exec_counts['reject_orders']} rejected "
                           f"({exec_counts['lot1013']}× LOT_SIZE/-1013). No no-fill / reject storm.")

    frames = [f for f in frames if not f.empty]
    if not frames:
        # Not a silent exit anymore: if the outcome probe found signals-but-no-fills, that IS the
        # finding — write it out and exit RED so the ML actually acts on the no-trade condition.
        if outcome_alerts:
            from datetime import date as _date
            out_path = Path(args.out) if args.out else Path("outputs") / f"ML_LOSS_MINING_REPORT_{_date.today().isoformat()}.md"
            out_path.parent.mkdir(parents=True, exist_ok=True)
            render_report([], out_path, notices)
            print(f"OUTCOME-RED: no closed trades to mine, but the executor shows "
                  f"{outcome_alerts[0]['code']} — surfaced instead of exiting silent.", file=sys.stderr)
            sys.exit(2)
    ...
    out_path.parent.mkdir(parents=True, exist_ok=True)
    render_report(results, out_path, notices)
