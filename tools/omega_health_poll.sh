...
set -uo pipefail
# CRON-PATH FIX (2026-06-30): cron runs with PATH=/usr/bin:/bin, which does NOT
# include Homebrew where `timeout` lives (/opt/homebrew/bin). Without this, every
# `timeout ... ssh` line died with "timeout: command not found" -> empty JSON ->
# false "VPS UNREACHABLE" alarm every 15min while the box was perfectly healthy.
# Prepend Homebrew, then resolve a usable timeout binary (no-op fallback if absent
# -- ssh's own ConnectTimeout still bounds the connection).
export PATH="/opt/homebrew/bin:/usr/local/bin:$PATH"
TIMEOUT="$(command -v timeout || command -v gtimeout || true)"
to(){ if [ -n "$TIMEOUT" ]; then "$TIMEOUT" "$@"; else shift; "$@"; fi; }  # to <secs> <cmd...>
