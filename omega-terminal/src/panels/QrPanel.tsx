// QrPanel — Quote Recap (multi-symbol tape).
//
// Step 6 deliverable. Sortable table over a comma-separated symbol list
// pulled from OpenBB Hub via `/api/v1/omega/qr`, which the engine-side
// OpenBbProxy forwards to /equity/price/quote?symbol=<list>.
//
// Args: `QR <symbol-list>` (comma-separated; spaces tolerated). Examples:
//   QR AAPL,MSFT,GOOG
//   QR SPY, QQQ, IWM
//
...
import { getQr } from '@/api/omega';
import type { OpenBbEnvelope, QuoteRow } from '@/api/types';
...
function joinSymbols(args: string[]): string {
  // The router uppercases args. Re-join so the user can type either
  // `QR AAPL,MSFT,GOOG` or `QR AAPL MSFT GOOG`.
  if (args.length === 0) return '';
  if (args.length === 1) return args[0]!.replace(/\s+/g, '');
  return args.map((a) => a.replace(/[, ]+/g, '')).filter(Boolean).join(',');
}

export function QrPanel({ args }: Props) {
  const symbols = joinSymbols(args);
  const [sortKey, setSortKey] = useState<SortKey>('change_percent');
  const [sortDir, setSortDir] = useState<SortDir>('desc');

  const fetcher = useCallback(
    (s: AbortSignal) => getQr({ symbols }, { signal: s }),
    [symbols],
  );
  const { state, refetch } = usePanelData<OpenBbEnvelope<QuoteRow>>(
    fetcher,
    [symbols],
    { pollMs: symbols ? QR_POLL_MS : 0 },
  );

  if (!symbols) {
    return <MissingSymbolPrompt code="QR" exampleArgs="AAPL,MSFT,GOOG" />;
  }
  ...
  return (
    ...
    >
      <PanelHeader
        ...
        subtitle={
          <>
            Multi-symbol tape via OpenBB &middot; symbols{' '}
          ...
          <tbody>
            ...
            {sorted.map((r) => (
              <tr key={r.symbol} className="border-b border-amber-900/40">
                ...
                <td className="px-3 py-2 text-right text-amber-400">
                  {r.bid != null ? formatNum(r.bid, 2) : '—'}
                </td>
                <td className="px-3 py-2 text-right text-amber-400">
                  {r.ask != null ? formatNum(r.ask, 2) : '—'}
                </td>
...
function sortRows(rows: QuoteRow[], key: SortKey, dir: SortDir): QuoteRow[] {
