...
def decide(rows: List[dict], aggregate: dict,
    ...
    lines.append("")
    if not overall:
        lines.append("  Implication: S63 baseline on XAU 4h trend-follow does NOT")
        lines.append("    clear the WF gate. Cross-reference the xtf_2h_wf_t1.py")
        lines.append("    verdict for the 2h sibling engine:")
        lines.append("      - If 2h also FAILED -> the S63-adverse pattern from VWR")
        lines.append("        USTEC (S71 P3) and UTF5m USTEC (S73 P3) generalises")
        lines.append("        across both XAU trend-follow timeframes. Engines stay")
        lines.append("        at class-default state-B values (S63 OFF).")
        lines.append("      - If 2h PASSED but 4h FAILED -> there is a timeframe-")
        lines.append("        specific shape inversion. Worth its own analysis")
        lines.append("        before generalising to the broader S63 universe sweep.")
        lines.append("    Next steps regardless:")
        lines.append("      (a) Run sibling driver scripts/xtf_d1_wf_t1.py (queued)")
        lines.append("          once it is created. Confirm the pattern across all")
        lines.append("          three XTF timeframes.")
        lines.append("      (b) Consider whether the Tier 4 vol-regime gate explored")
        lines.append("          in the VWR/UTF5m scoping memos applies to XTF too.")
    else:
