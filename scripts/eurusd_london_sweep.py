#!/usr/bin/env python3
"""
eurusd_london_sweep.py
======================
Parameter sweep driver for EurusdLondonOpenEngine.

Built per audit-fixes-38b (see docs/SESSION_2026-05-04_FX_BACKTEST_AUDIT.md
Section 8.1) to backstop the EURUSD reproducibility gap. Mirrors the
methodology used for the USDJPY S59 sweeps:
  1. For each parameter / value pair, sed-rewrite a tmp copy of the engine
     header in backtest/eurusd_bt/EurusdLondonOpenEngine.hpp.
  2. Recompile the harness (build/eurusd_london_bt).
  3. Run against the staged 14-month EURUSD HistData tick set.
  4. Aggregate per-run summary into one results CSV.
  5. Print a sorted leaderboard.

The production engine in include/EurusdLondonOpenEngine.hpp is never
modified.
"""
...
INC    = REPO / "include" / "EurusdLondonOpenEngine.hpp"
BT_DIR = REPO / "backtest" / "eurusd_bt"
BT_HPP = BT_DIR / "EurusdLondonOpenEngine.hpp"
BT_CPP = BT_DIR / "EurusdLondonOpenBacktest.cpp"
BIN    = REPO / "build" / "eurusd_london_bt"
# Allow override via env var. Default to ~/Tick/EURUSD on Mac.
TICKS  = os.environ.get("EURUSD_TICKS", os.path.expanduser("~/Tick/EURUSD"))
RESULTS_CSV = REPO / "build" / "eurusd_sweep_results.csv"

# Default parameter values matching the production engine.hpp at port time.
# These are the S56 chosen-winner values from the EURUSD handoff doc.
...
# Run the harness; return summary_dict.
def run_bt(label, frm="2025-03", to="2026-04", trades_csv=None):
    if trades_csv is None:
        trades_csv = REPO / "build" / f"sweep_eurusd_trades_{label}.csv"
    report_csv = REPO / "build" / f"sweep_eurusd_report_{label}.csv"
    cmd = [str(BIN),
           "--ticks", TICKS,
           "--from", frm, "--to", to,
...
def _print_row(label, s):
    """Standard one-liner used by the walkforward driver too."""
...
# -- Single-axis sweep specification (FX-direct-quote pip scale) ------------
SWEEPS = [
    ...
    ("SAME_LEVEL_BLOCK_PTS", [0.0004, 0.0006, 0.0008, 0.0010, 0.0012]),
]
