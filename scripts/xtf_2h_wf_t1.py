...
def decide(rows: List[dict], aggregate: dict,
    ...
    lines.append("")
    if not overall:
        lines.append("  Implication: S63 baseline on XAU 2h trend-follow does NOT")
        lines.append("    clear the WF gate. The S63-adverse pattern from VWR USTEC")
        lines.append("    (S71 P3) and UTF5m USTEC (S73 P3) appears to generalise to")
        lines.append("    XAU trend-follow at the 2h timeframe -- a notable finding")
        lines.append("    given XAU's different tail shape. Engines remain at class-")
        lines.append("    default state-B values (S63 OFF). Next session should:")
        lines.append("      (a) Run sibling drivers xtf_4h_wf_t1.py + xtf_d1_wf_t1.py")
        lines.append("          once those harness paths are filled in. Confirm the")
        lines.append("          pattern across all three timeframes.")
